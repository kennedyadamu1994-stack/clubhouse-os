 ...
 *
 * Real login gate as of 5 Sep (security remediation, step 2) —
 * requireAdminOrRedirect() sends anyone without a valid session to
 * /admin/login. Was genuinely unprotected before this (relying only on
 * this URL not being linked from the club-facing app), which is why the
 * three dangerous actions below (reset balance, rotate token, clear all
 * logs) ALSO check the session themselves now, independent of this
 * page's own guard — a server action is directly callable regardless of
 * whether its page was ever visited, so gating only the page was never
...
function resolveEntryDetail(
  ...
  lookups: {
    ...
    socialVenues: Map<string, SocialVenue>;
  },
): string | null {
  ...
  switch (row.action_key) {
    ...
    case "club_outreach": {
      // Clubs only (7 Sep fix) — Social Venues used to share this exact
      // key too (Kennedy's report: "the social venue outreach... is
      // logging the action as a club to club action"), which meant a
      // venue enquiry's entry_id (a real venue_id) was being looked up
      // here, against the CLUBS directory, and would resolve to nothing,
      // or worse, silently match an unrelated club that happened to
      // share the same ID. Social Venues now has its own real
      // "venue_outreach" key (case below) with its own lookup.
      const c = lookups.clubsDirectory.get(row.entry_id);
      return c?.name ?? null;
    }
    case "venue_outreach": {
      const v = lookups.socialVenues.get(row.entry_id);
      return v?.name ?? null;
    }
    ...
    case "service_enquiry": {
      const s = lookups.services.get(row.entry_id);
      return s?.name ?? null;
    }
    case "supplier_outreach": {
      const s = lookups.suppliers.get(row.entry_id);
      return s?.name ?? null;
    }
    default:
      // contact_us / feature_request / list_callout / view_opportunity /
      // view_resource genuinely have no single "entry" to resolve — these
      // are either free-text-only submissions (detail lives in notes, if
      // the club filled it in) or plain link clicks with no real ID
      // (list_callout's entryId is a hardcoded "callout" placeholder, not
      // a lookup-able ID). service_enquiry and supplier_outreach used to
      // fall through here too — that was the actual bug (Kennedy, 6 Sep:
      // "it shows me the club but it doesn't specify what service they
      // are after") — both now have their own case above, resolving the
      // real entry_id that was already being recorded the whole time.
      return null;
  }
}
...
export default async function AdminRequestsPage() {
  ...
  const db = getAdapter();
  const [actions, clubs, tokenRefs, players, people, brands, influencers, clubsDirectory, sponsorships, services, suppliers, socialVenues, playerNotifications] =
    await Promise.all([
      db.getAllActions(),
      ...
      db.getSuppliers(),
      db.getSocialVenues(),
      // POS section (18 Sep, Kennedy: "create a POS section in the data
      // log, so that I can see player requests") — was its own separate,
      // unlinked page (app/admin/player-requests/page.tsx); that page's
      // read (P NOTIFICATIONS, see getPlayerNotifications's own doc
      // comment in lib/data/sheets.ts) is now pulled into this page
      // directly instead, so Kennedy sees both CHOS and POS requests in
      // one place without needing to know a second URL exists.
      db.getPlayerNotifications(),
    ]);
  ...
  const lookups = {
    ...
    suppliers: new Map(suppliers.map((s) => [s.supplier_id, s])),
    socialVenues: new Map(socialVenues.map((v) => [v.venue_id, v])),
  };
  ...
  return (
    <main style={{ minHeight: "100vh", padding: "48px 24px" }}>
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
            ...
            items={balances.map(({ club, balance, allocation }) => (
              ...
              >
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                  ...
                  />
                  <RotateTokenButton clubId={club.club_id} clubName={club.name} currentToken={club.club_token} />
                </div>
              </div>
            ))}
          />
        ...
        {/* POS section (18 Sep) — player Contact Us / Request a Feature
            submissions, read from the P NOTIFICATIONS sheet. Deliberately
            its own section rather than merged into AdminRequestsBoard
            above: that board is built entirely around club token/status
            workflows (pending/logged toggle, token cost, entry lookups
            across nine club-side entities) a player submission has none
            of — same reasoning as when this was still a separate page,
            just relocated here now instead of living at a second URL. */}
        <div style={{ marginTop: 48, paddingTop: 24, borderTop: "1px solid var(--line)" }}>
          <h2 style={{ marginBottom: 8 }}>
            POS <em style={{ color: "var(--pink)", fontStyle: "normal" }}>Player Requests</em>
          </h2>
          <p style={{ color: "var(--dim)", marginBottom: 20, fontSize: "0.9rem" }}>
            Contact Us and Request a Feature submissions from POS, newest first.
          </p>
          {playerNotifications.length === 0 ? (
            <div className="card" style={{ padding: 24, color: "var(--dim)" }}>
              No player submissions yet.
            </div>
          ) : (
            <PaginatedList
              className="entry-list"
              items={playerNotifications.map((n, i) => (
