 ...
 * SAME already-gated reads (getPlayers/getPeople/etc, CLAUDE.md rule 3) the
 * rest of the app uses — never a second, ungated lookup.
 *
 * Player names are NEVER shown (Kennedy, 20 Aug: a blanket policy, not
 * conditional on consent) — every player always shows via record ID
 * instead, so Kennedy can look them up in the real database if needed.
 * People (professionals) are different: their name shows unless that
 * specific person withheld consent, in which case the same ID-fallback
 * pattern applies. Two different rules, deliberately not shared logic.
 */
function resolveEntryDetail(
  ...
  switch (row.action_key) {
    case "player_invite": {
      ...
      if (!p) return null;
      // Always the fallback — p.name is always null now (gatePlayer),
      // never a consent outcome, so the wording doesn't reference consent.
      return `Player in ${p.area}, ID ${p.player_id}`;
    }
    case "person_request": {
