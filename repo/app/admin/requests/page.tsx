 ...
 * Player/Person's name is null (no sharing consent), shows their record ID
 * instead — Kennedy can look that ID up in the real database if he needs to
 * follow up, without this page ever displaying a name the person withheld
 * consent for. The gate on the NAME itself is never bypassed; the ID is not
 * gated data (CLAUDE.md rule 3 protects names/contact details, not the
 * record identifier itself).
 */
function resolveEntryDetail(
  ...
  switch (row.action_key) {
    case "player_invite": {
      ...
      if (!p) return null;
      return p.name ?? `Player in ${p.area}, ID ${p.player_id} (name withheld — no sharing consent)`;
    }
    case "person_request": {
