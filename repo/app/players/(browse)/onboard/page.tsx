...
export default function OnboardPage() {
  ...
  async function handleSubmit() {
    ...
    // forceFresh: true — real fix (17 Sep, Kennedy: "once someone has
    // completed onboarding, they should just go back to the homepage
    // signed in" — previously redirected to /players/[token]/welcome,
    // which called the not-found page instead of a real page). The
    // token'd welcome flow itself is untouched elsewhere; this is
    // specifically about what onboarding's own success path does.
    // Signs the brand-new player in immediately using the email they
    // just submitted, bypassing the Sheets read cache (see login's own
    // doc comment, components/player-session.tsx, and readTab's, lib/
    // data/sheets/client.ts, for the real race this guards against:
    // NEIGHBOURS (O) may not have pulled the new row through its own
    // array formula yet). If login still fails even with a fresh read
    // — e.g. the formula itself hasn't run yet — Home's own sign-in
    // card is still right there for a manual retry, so this never
    // strands anyone on a dead page either way.
    await login(form.email, true);
    router.push("/players");
  }
