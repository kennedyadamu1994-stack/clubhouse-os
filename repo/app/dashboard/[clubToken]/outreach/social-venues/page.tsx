...
export default async function SocialVenuesOutreach({
  ...
  const [venues, actions, favouritedKeys] = await Promise.all([
    db.getSocialVenues(),
  ...
  const entries: OutreachEntry[] = venues.map((v) => {
    ...
    return {
      ...
      sortValues: { rating: v.rating ?? -1, quality: v.credibility_score ?? -1 },
      nameForSort: v.name,
      ...
      card: (
        <EntryCard
          ...
          sponsored={v.sponsored}
          credibilityScore={v.credibility_score}
          favouriteCategory="social_venue"
          favouriteHref={`${base}/outreach/social-venues`}
          favouritedKeys={favouritedKeys}
          detail={[
            ...(v.venue_type ? [{ label: "Venue type", value: v.venue_type }] : []),
            ...
            ...(v.best_for.length > 0 ? [{ label: "Best for", value: v.best_for.join(", ") }] : []),
            ...(v.venue_description_long ? [{ label: "About", value: v.venue_description_long }] : []),
            ...(v.website_url ? [{ label: "Website", value: v.website_url, href: v.website_url }] : []),
            ...(v.instagram_handle
              ? [
                  {
                    label: "Instagram",
              ...
              : []),
            ...(v.contact_email ? [{ label: "Email", value: v.contact_email, href: `mailto:${v.contact_email}` }] : []),
            ...(v.contact_phone ? [{ label: "Phone", value: v.contact_phone }] : []),
            ...(v.verification_status ? [{ label: "Verification", value: v.verification_status }] : []),
          ]}
          consentNote={
