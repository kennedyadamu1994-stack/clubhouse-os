...
export default async function SuppliersOutreach({
  ...
  const [suppliers, actions, favouritedKeys] = await Promise.all([
    db.getSuppliers(),
  ...
  const entries: OutreachEntry[] = suppliers.map((s) => {
    return {
      ...
      sortValues: { rating: s.credibility_score ?? -1 },
      nameForSort: s.name,
      ...
      card: (
        <EntryCard
          ...
          favouriteCategory="supplier"
          favouriteHref={`${base}/outreach/suppliers`}
          favouritedKeys={favouritedKeys}
          detail={[
            ...(s.product_categories.length > 0
              ? [{ label: "Product categories", value: s.product_categories.join(", ") }]
              : []),
            ...(s.brands_carried.length > 0 ? [{ label: "Brands carried", value: s.brands_carried.join(", ") }] : []),
            ...(s.price_tier ? [{ label: "Price tier", value: s.price_tier }] : []),
            ...
            ...(s.delivery_area ? [{ label: "Delivery area", value: s.delivery_area }] : []),
            ...(s.past_grassroots_clients
              ? [{ label: "Past grassroots clients", value: s.past_grassroots_clients }]
              : []),
            ...(s.contact ? [{ label: "Contact", value: s.contact, href: `mailto:${s.contact}` }] : []),
