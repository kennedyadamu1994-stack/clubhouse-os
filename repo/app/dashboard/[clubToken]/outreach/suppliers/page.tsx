 ...
 * full column mapping and lib/data/sheets.ts's getSuppliers() for how
 * each real column is read.
 *
 * Locked on the Free plan, matching Brands/Influencers/Sponsorship's
 * precedent (25 Aug correction) — suppliers are a commercial-partner
 * category, not a Players/People/Clubs one.
 *
...
export default async function SuppliersOutreach({
  ...
  if (!hasPaidAccess(club.plan_tier)) {
    return <PaywallGate sectionName="Suppliers" plans={await db.getPlans()} />;
  }
