...
export default async function ClubsOutreach({
  ...
  const [directoryRaw, actions, favouritedKeys] = await Promise.all([
    db.getClubsDirectory(),
    db.getActionsForClub(club.club_id),
    db.getFavouritedKeys(club.club_id),
  ]);
  // Never show a club its own directory row as an outreach target.
  ...
  const entries: OutreachEntry[] = directory.map((c) => {
    ...
    // (Sport/Location/Audience/Gender). ClubDirectoryEntry has no
    // dedicated audience/gender fields, so those two are classified from
    // c.tags alone inside buildReasonChips.
    const chips = buildReasonChips(club, { sport: c.sport, area: c.area, tags: c.tags });
    return {
      key: c.directory_id,
      searchText: [c.name, c.sport, c.area, c.open_to.join(" ")].join(" "),
      filterValues: { sport: c.sport, area: c.area },
      sortValues: { match: relevanceSortValue(chips), rating: c.credibility_score ?? -1 },
      nameForSort: c.name,
      ...
      card: (
        <EntryCard
          ...
          sponsored={c.sponsored}
          credibilityScore={c.credibility_score}
          favouriteCategory="club"
          favouriteHref={`${base}/outreach/clubs`}
          favouritedKeys={favouritedKeys}
          verified={c.verified}
          detail={[
            { label: "Sport", value: c.sport },
            { label: "Area", value: c.area },
            ...(c.open_to.length > 0 ? [{ label: "Open to", value: c.open_to.join(", ") }] : []),
            ...
            ...(c.website_url ? [{ label: "Website", value: c.website_url, href: c.website_url }] : []),
            ...(c.instagram_url ? [{ label: "Instagram", value: c.instagram_url, href: c.instagram_url }] : []),
