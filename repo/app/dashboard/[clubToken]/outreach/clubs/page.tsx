...
export default async function ClubsOutreach({
  ...
  // Locked on the Free plan (Kennedy's request, 25 Aug).
  if (!hasPaidAccess(club.plan_tier)) {
    return <PaywallGate sectionName="Clubs" plans={await db.getPlans()} />;
  }
  ...
  const entries: OutreachEntry[] = directory.map((c) => {
    const score = matchScore(club, { sports: [c.sport], area: c.area, tags: c.open_to }, weights);
    return {
      key: c.directory_id,
      ...
      card: (
        <EntryCard
          ...
          actions={[
            ...
            { action_key: "club_outreach", label: `We'll set it up with ${c.name}`, colour: "pink", token_cost: 1 },
          ]}
