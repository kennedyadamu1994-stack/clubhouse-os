...
export default async function PeopleOutreach({
  ...
  const [people, actions, favouritedKeys] = await Promise.all([
    db.getPeople(),
  ...
  const entries: OutreachEntry[] = people.map((p) => {
    ...
    return {
      ...
      sortValues: { rating: p.credibility_score ?? -1 },
      ...
      card: (
        <EntryCard
          ...
          favouriteCategory="person"
          favouriteHref={`${base}/outreach/people`}
          favouritedKeys={favouritedKeys}
          verified={p.verified}
          detail={[
            { label: "Role", value: label },
            ...(p.sports.length > 0 ? [{ label: "Sports", value: p.sports.join(", ") }] : []),
            ...(p.availability ? [{ label: "Availability", value: p.availability }] : []),
            ...(p.rate_note ? [{ label: "Notes", value: p.rate_note }] : []),
            ...(p.bio ? [{ label: "Bio", value: p.bio }] : []),
          ]}
          consentNote={
            p.name
              ? undefined
