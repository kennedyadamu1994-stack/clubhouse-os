...
export default async function SponsorshipOutreach({
  ...
  // Tags-based match score (Kennedy, 29 Aug) — sort by match first,
  // closing date as the tiebreaker.
  const initialOrder = [...sponsorships].sort((a, b) => {
    const diff = tagMatchScore(club, a.tags) - tagMatchScore(club, b.tags);
  ...
  const entries: OutreachEntry[] = initialOrder.map((sp) => {
    const score = tagMatchScore(club, sp.tags);
