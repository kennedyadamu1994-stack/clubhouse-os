...
export default async function PlayersOutreach({
  ...
  // Locked on the Free plan (Kennedy's request, 25 Aug) — returns before
  // fetching real player data at all, so a Free club's request never even
  // reads the (potentially sensitive) player list.
  if (!hasPaidAccess(club.plan_tier)) {
    return <PaywallGate sectionName="Players" plans={await db.getPlans()} />;
  }
  ...
  const entries: OutreachEntry[] = players.map((p) => {
    const score = matchScore(club, { sports: p.sports, area: p.area, tags: p.interests }, weights);
    return {
      key: p.player_id,
