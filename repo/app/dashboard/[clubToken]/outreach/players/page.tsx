...
import { clubToPlayerMatchScore } from "@/lib/scoring";
...
export default async function PlayersOutreach({
  ...
  const [players, actions, directory] = await Promise.all([
    db.getPlayers(),
  ...
  const entries: OutreachEntry[] = players.map((p) => {
    // Club-to-player scoring card (Kennedy, 27 Aug) — six fields, out of
    // 50, shown as a percentage. Replaces the generic sport/area/urgency
    // matchScore() for this page specifically.
    const score = clubToPlayerMatchScore(club, p, directory);
    return {
      key: p.player_id,
