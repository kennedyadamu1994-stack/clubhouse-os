...
export default async function PlayersOutreach({
  ...
  const [players, actions, favouritedKeys] = await Promise.all([
    db.getPlayers(),
  ...
  const entries: OutreachEntry[] = players.map((p) => {
    ...
    return {
      ...
      sortValues: { match: relevanceSortValue(chips), rating: p.credibility_score ?? -1 },
      ...
      card: (
        <EntryCard
          ...
          favouriteCategory="player"
          favouriteHref={`${base}/outreach/players`}
          favouritedKeys={favouritedKeys}
          detail={[
            ...(p.area ? [{ label: "Area", value: p.area }] : []),
            ...(p.sports.length > 0 ? [{ label: "Sports", value: p.sports.join(", ") }] : []),
            ...(p.interests.length > 0 ? [{ label: "Interests", value: p.interests.join(", ") }] : []),
            ...(p.preferred_times ? [{ label: "Preferred times", value: p.preferred_times }] : []),
            ...(p.level ? [{ label: "Level", value: p.level }] : []),
            ...(p.gender ? [{ label: "Gender", value: p.gender }] : []),
            ...(p.age != null ? [{ label: "Age", value: String(p.age) }] : []),
          ]}
          consentNote={
