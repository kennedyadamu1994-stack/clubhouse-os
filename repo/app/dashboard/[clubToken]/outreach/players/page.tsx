...
export default async function PlayersOutreach({
  ...
  const entries: OutreachEntry[] = players.map((p) => {
    // Tags-based match score (Kennedy, 29 Aug) — replaces the old
    // point-based clubToPlayerMatchScore entirely, per Kennedy's explicit
    // decision to discard the PDF-based systems rather than layer tags on
    // top of them.
    const score = tagMatchScore(club, p.tags);
    return {
      key: p.player_id,
