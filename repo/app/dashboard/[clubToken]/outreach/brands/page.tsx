...
export default async function BrandsOutreach({
  ...
  const [brands, actions, favouritedKeys] = await Promise.all([
    db.getBrands(),
  ...
  const entries: OutreachEntry[] = brands.map((b) => {
    ...
    return {
      ...
      sortValues: { match: relevanceSortValue(chips), rating: b.credibility_score ?? -1 },
      nameForSort: b.name,
      ...
      card: (
        <EntryCard
          ...
          favouriteCategory="brand"
          favouriteHref={`${base}/outreach/brands`}
          favouritedKeys={favouritedKeys}
          detail={[
            ...(b.area ? [{ label: "Area", value: b.area }] : []),
            { label: "Type", value: TYPE_LABEL[b.type] },
            ...(b.sectors.length > 0 ? [{ label: "Sectors", value: b.sectors.join(", ") }] : []),
            ...(b.partnership_interests.length > 0
              ? [{ label: "Interested in", value: b.partnership_interests.join(", ") }]
              : []),
            ...(b.marketing_budget_signal
              ? [{ label: "Marketing budget signal", value: b.marketing_budget_signal }]
              : []),
            ...(b.csr_esg_sport_mandate
              ? [{ label: "CSR/ESG sport mandate", value: b.csr_esg_sport_mandate }]
              : []),
            ...(b.contact ? [{ label: "Contact", value: b.contact, href: `mailto:${b.contact}` }] : []),
          ]}
          consentNote={
            b.contact
              ? undefined
