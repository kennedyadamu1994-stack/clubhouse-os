...
export default async function BrandsOutreach({
  ...
  if (!hasPaidAccess(club.plan_tier)) {
    return <PaywallGate sectionName="Brands & Businesses" plans={await db.getPlans()} />;
  }
  ...
  const entries: OutreachEntry[] = brands.map((b) => {
    const score = matchScore(club, { area: b.area, tags: b.partnership_interests }, weights);
    return {
      key: b.brand_id,
      ...
      card: (
        <EntryCard
          ...
          actions={[
            { action_key: "visit_website", label: `Visit ${b.name}'s website`, colour: "black", token_cost: 0, href: b.website },
            {
