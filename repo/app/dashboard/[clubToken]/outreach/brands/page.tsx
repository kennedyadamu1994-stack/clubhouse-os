...
export default async function BrandsOutreach({
  ...
  const entries: OutreachEntry[] = brands.map((b) => {
    // v3 reason chips (Kennedy, 1 Sep) — replaces the old tags-based
    // matchScore entirely. Uses the real "tags" column, not
    // partnership_interests (a different real field, still used for
    // search/filter above, unaffected). No sport field on Brand, so this
    // entry type can only ever earn an area chip plus tag chips.
