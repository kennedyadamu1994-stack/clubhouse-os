...
export default async function InfluencersOutreach({
  ...
  const [influencers, actions, favouritedKeys] = await Promise.all([
    db.getInfluencers(),
  ...
  const entries: OutreachEntry[] = influencers.map((inf) => {
    // No match score here (Kennedy's request, 31 Aug) — removed after
    // being added 29 Aug.
    return {
      ...
      sortValues: { followers: parseFollowerCount(inf.follower_band), rating: inf.credibility_score ?? -1 },
      ...
      card: (
        <EntryCard
          ...
          sponsored={inf.sponsored}
          credibilityScore={inf.credibility_score}
          favouriteCategory="influencer"
          favouriteHref={`${base}/outreach/influencers`}
          favouritedKeys={favouritedKeys}
          detail={[
            ...(inf.platforms.length > 0 ? [{ label: "Platforms", value: inf.platforms.join(", ") }] : []),
            ...(inf.follower_band ? [{ label: "Followers", value: inf.follower_band }] : []),
            ...(inf.niches.length > 0 ? [{ label: "Focus", value: inf.niches.join(", ") }] : []),
            ...(inf.area ? [{ label: "Area", value: inf.area }] : []),
          ]}
          actions={[
