...
export default async function Overview({ params }: { params: Promise<{ clubToken: string }> }) {
  ...
  const recs = topRecommendations(club, sponsorships, opportunities, players);
  ...
  return (
    <>
      ...
      <div className="grid">
        ...
        {/* Top recommendations, rebuilt onto the v3 chip system (1 Sep follow-up): only sponsorships/opportunities with 2+ matching reason chips qualify at all (topRecommendations in lib/scoring.ts), sorted by chip count then nearest date. Can legitimately be empty if nothing clears that bar yet. */}
        <CollapsibleCard
          id="recs"
          defaultOpen={false}
          heading={
