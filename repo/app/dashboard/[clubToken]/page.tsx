...
export default async function Overview({ params }: { params: Promise<{ clubToken: string }> }) {
  ...
  const [weights, sponsorships, opportunities, players, actions, asOf, balanceInfo, plans, nbrhUpdates] =
    await Promise.all([
      getScoreWeights(),
      db.getSponsorships(),
      db.getOpportunities(),
    ...
    <>
      ...
      <div className="grid">
        ...
        {/* KPIs — visual language matches the reference "Performance" cards (accent top
            ...
            template. */}
        <CollapsibleCard id="kpis" heading="Your KPIs" defaultOpen={false}>
          {club.kpis.length > 0 ? (
        ...
        {club.connected_pages && club.connected_pages.length > 0 && (
          <CollapsibleCard id="connected-pages" className="span2" defaultOpen={false} heading="Connected Pages">
        ...
        {/* From The NBRH — real feed from CHOS Workspace's "FROM THE NBRH"
            ...
            decision. */}
        <CollapsibleCard id="note" className="span2 note-card" defaultOpen={false} heading="From The NBRH">
