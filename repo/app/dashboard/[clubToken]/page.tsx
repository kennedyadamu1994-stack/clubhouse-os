...
export default async function Overview({ params }: { params: Promise<{ clubToken: string }> }) {
  ...
  const recs = topRecommendations(club, sponsorships, players, people, brands, influencers, directory, suppliers, socialVenues);
      ...
      actions: [
        ...(v.booking_url
          ...
          : v.website_url
            ...
            : []),
        { action_key: "venue_outreach", label: `We'll set it up with ${v.name}`, colour: "pink" as const, token_cost: 2 },
      ],
  ...
  // Filter the global search index for Free clubs (Kennedy, 1 Sep) so it
  // can't surface — or link into — sections that are otherwise hidden and
  // route-blocked: Workspace (Resource/FAQ/Perk/Trending) and Tools
  // categories are dropped entirely, and Outreach's Person/Brand/
  // Influencer/Supplier/"Social Venue" categories are dropped too, since
  // only Player/Club/Sponsorship stay open for Free. Every href above
  // already points at a real page, and several of those pages now 404
  // for Free — this keeps the search bar from ever linking there in the
  // first place, rather than surfacing a dead link.
  const FREE_HIDDEN_CATEGORIES = new Set<SearchIndexEntry["category"]>([
    "Person",
    "Brand",
    "Influencer",
    "Supplier",
  ...
  const visibleSearchIndex =
    club.plan_tier === "free" ? searchIndex.filter((e) => !FREE_HIDDEN_CATEGORIES.has(e.category)) : searchIndex;

  return (
    <>
      <OverviewSearchBar
        index={visibleSearchIndex}
        clubToken={clubToken}
      ...
      <div className="grid">
        {/* Club Health + KPIs sit side by side on wide screens (18 Sep,
            design pass) — previously both stretched to the full,
            genuinely unbounded width of .main (Kennedy's 27 Aug no-
            max-width decision) as two separate full-width rows, even
            though Club Health's own content (one big % stat + a
            ranking + five short metric bars) and KPI's (three small
            cards) never needed that much horizontal room — a
            hierarchy/proximity gap per the two design-principle
            reports this pass is built from: two naturally compact
            sections were each claiming a full row for no reason,
            pushing everything below them further down the page than
            it needed to be. Weighted 3:2 rather than an even split —
            Club Health carries the headline number and is the more
            important of the two, KPIs is supporting detail — via
            .overview-top-row below. Stacks back to one column under
            980px, where there's no longer room to keep both legible
            side by side. */}
        <div className="overview-top-row">
          <CollapsibleCard id="club-health" heading="Club Health" className="overview-top-row-health">
            {club.club_health ? (
              <ClubHealthCard
                health={club.club_health}
                sectorRanking={club.sector_ranking}
                sectorCategory={club.sector_category}
              />
            ) : (
              <>
          ...
          {/* KPIs — visual language matches the reference "Performance" cards (accent top
              ...
              template. */}
          <CollapsibleCard id="kpis" heading="Your KPIs" className="overview-top-row-kpis">
            {club.kpis.length > 0 ? (
        ...
        {/* Top recommendations, fully manual now (8 Sep — Kennedy: replaces the old automatic chip-matching entirely). Kennedy tags any row on any of the 8 Outreach sheets with a club's real Club ID in that row's "U notes" cell (topRecommendations in lib/scoring.ts, parseRecommendedForClubIds does the actual parsing) — only rows naming this club show here. Can legitimately be empty if nothing's been tagged for this club yet. */}
        <CollapsibleCard
          id="recs"
          defaultOpen={false}
          heading={
        ...
        >
          {recs.length > 0 ? (
            ...
            <p style={{ color: "var(--dim)", maxWidth: "60ch" }}>
              No recommendations yet. Check back once something's been tagged for your club.
            </p>
          )}
        </CollapsibleCard>
