...
export default async function Overview({ params }: { params: Promise<{ clubToken: string }> }) {
  ...
  const [weights, sponsorships, opportunities, players, actions, asOf, balanceInfo, plans, nbrhUpdates, resources, faq] =
    await Promise.all([
      getScoreWeights(),
      db.getSponsorships(),
      db.getOpportunities(),
  ...
  // Universal search index (Kennedy's request, 27 Aug fourth follow-up —
  // the search bar was previously a static, non-functional input). Every
  // player entry uses the same privacy-respecting title as the Players
  // page itself ("Player in <area>", never the real name — see
  // gatePlayer's own doc comment in lib/data/index.ts) since players are
  // always anonymised regardless of where they're surfaced in the app.
  const searchIndex: SearchIndexEntry[] = [
    ...
    ...resources.map((r) => ({
      ...
      searchText: [r.title, r.category, r.summary].join(" ").toLowerCase(),
    })),
    ...faq.map((f) => ({
      id: `faq-${f.faq_id}`,
      title: f.question,
  ...
  return (
    <>
      <OverviewSearchBar index={searchIndex} />
