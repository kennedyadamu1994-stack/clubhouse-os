...
/**
 * Looser than anyOverlap — true if any value in a appears as a substring
 * of any value in b or vice versa (case/whitespace-insensitive). Used
 * ONLY for location matching, where two real, correct-in-spirit values
 * routinely fail an exact match: "Hackney" vs "East Hackney" vs "Hackney
 * Wick", "London" vs "East London". An exact-match rule here would
 * silently zero out the location points for genuinely-nearby clubs far
 * more often than it should, which was part of why the original version
 * of this scoring under-delivered (Kennedy, 27 Aug follow-up: "the match
 * score doesn't really work"). Every other field on both cards
 * (activity_type, audience) keeps exact-overlap matching, since those
 * are closed-ish category values (a sport, an audience label) where a
 * substring match risks false positives rather than fixing anything.
 */
const anyLooseOverlap = (a: string[], b: string[]) => {
  const av = a.map((x) => x.toLowerCase().trim()).filter(Boolean);
  const bv = b.map((x) => x.toLowerCase().trim()).filter(Boolean);
  return av.some((x) => bv.some((y) => x.includes(y) || y.includes(x)));
};

/**
 * A club's own activity_type/location/audience for the two match-score
 * cards (Kennedy's PDFs, 27 Aug). Redesigned 27 Aug follow-up after
 * Kennedy reported "the match score doesn't really work" — the original
 * version sourced ALL THREE fields from the club's own row in the
 * Dynamic Club Page Hub directory, which most clubs (including the
 * pilot club, both locally and confirmed against the real sheet) don't
 * have a row in — that sheet is built for OTHER clubs to browse, not as
 * a profile store every club is guaranteed to appear in. The result was
 * every match score silently computing to 0% for every real club, which
 * is indistinguishable from "not working" even though the maths itself
 * was correct.
 *
 * Fixed by sourcing activity_type/location from Club.sport/Club.area
 * instead — the same two fields DASHBOARD (X) populates for every club
 * from onboarding, so they're always present. audience has no DASHBOARD
 * (X) equivalent (a documented, pre-existing gap), so it still comes
 * from the Dynamic Club Page Hub directory lookup where available, and
 * is simply treated as unknown (no audience points awarded, other
 * fields unaffected) when a club has no row there — a club's score can
 * now be meaningfully non-zero even with audience unknown, rather than
 * the whole score collapsing because one of three fields was missing.
 */
export function ownClubMatchFields(
  club: Club,
  ...
  return {
    activity_type: club.sport || own?.sport || "",
    location: club.area || own?.area || "",
    audience: own?.open_to ?? [], // no DASHBOARD (X) equivalent — enrichment-only, see doc comment above
  };
}
 ...
 * Location uses anyLooseOverlap (substring match) rather than exact —
 * see that function's doc comment for why. Returns a 0–100 PERCENTAGE
 * (points scored ÷ 25 × 100, rounded), not a raw point count — Kennedy's
 * own example: "10 out of 15" (his 25-point total, confirmed 27 Aug —
 * the 15 in his message was a typo) → the UI always shows a percentage,
 * never "10/25".
 */
export function clubToClubMatchScore(myClub: Club, otherClub: ClubDirectoryEntry, directory: ClubDirectoryEntry[]): number {
  const mine = ownClubMatchFields(myClub, directory);
  const MAX = 25;
  let points = 0;
  if (mine.activity_type && anyOverlap([mine.activity_type], [otherClub.sport])) points += 10;
  if (mine.location && anyLooseOverlap([mine.location], [otherClub.area])) points += 5;
 ...
 * tags↔Motivations 5, U notes↔Experience Level 10. Location uses
 * anyLooseOverlap (substring match) — see that function's doc comment.
 *
...
export function clubToPlayerMatchScore(myClub: Club, player: Player, directory: ClubDirectoryEntry[]): number {
  ...
  if (mine.location && anyLooseOverlap([mine.location], [player.area])) points += 10;
