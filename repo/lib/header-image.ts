...
/**
 * Demo set for the header carousel (Kennedy's request, 12 Aug: "for now,
 * have 5 images"). Deliberately separate from headerImageFor/FALLBACKS
 * above — that function's job (one photo per club, real or sport-fallback)
 * is unchanged; this is a placeholder set to test the carousel mechanism
 * itself. Reuses 5 of the FALLBACKS photos above rather than sourcing new
 * ones — those are already live in production, so they're known-working
 * URLs rather than newly-guessed ones. When Kennedy has real per-club
 * photos, swap this array for however many the club actually has (the
 * carousel component itself already handles any array length, including 1).
 */
export const DEMO_CAROUSEL_IMAGES: string[] = [
  FALLBACKS.football,
  FALLBACKS.netball,
  FALLBACKS.basketball,
  FALLBACKS.cricket,
  FALLBACKS.rugby,
];
