 ...
 */
/**
 * Exported (15 Sep, POS Contact Us / Request a Feature) — additive
 * only, this function's own write logic is completely unchanged.
 * lib/players/actions.ts's own submitPlayerAction reuses this directly
 * rather than duplicating the NOTIFICATIONS-tab write logic a second
 * time, since it's the exact same real sheet and format either way —
 * only the "who this came from" label differs (a player's name/email
 * instead of a club name).
 *
 * `tab` param added (18 Sep, Kennedy: "I've created another worksheet
 * in the same workbook called 'P NOTIFICATIONS', this is exclusively
 * for players... connect this worksheet to the POS") — defaults to the
 * original "NOTIFICATIONS" tab so every existing club call site is
 * unchanged; lib/players/actions.ts's submitPlayerAction is the one
 * caller that now passes "P NOTIFICATIONS" explicitly. Same workbook
 * ("workspace"), same header shape, just a different tab per Kennedy's
 * own split — player rows no longer share NOTIFICATIONS with club rows
 * at all, so the old "action label starts with player_" filter in
 * getPlayerNotifications is no longer needed for new rows.
 */
export async function notifyKennedyOfAction(input: {
  clubName: string;
  action_key: string;
  notes?: string;
  tab?: string;
}) {
  const tab = input.tab ?? "NOTIFICATIONS";
  const db = getAdapter();
  const [tokensRef, existingRows] = await Promise.all([
    db.getTokensReference(),
    // forceFresh: true (19 Sep fix, real bug — see appendRow's own doc
    // comment in lib/data/sheets/client.ts for the full diagnosis). This
    // check decides whether to write a header row, so it must never
    // answer from readTab's 5-minute cache: two submissions landing
    // within the same cache window on a still-young tab could otherwise
    // both see a stale "just the header" (or genuinely empty) state and
    // both try to bootstrap — which, combined with the missing
    // INSERT_ROWS fixed alongside this, is what let a second submission
    // overwrite the first instead of landing as a new row below it.
    readTab("workspace", tab, true).catch(() => []),
  ]);
  if (existingRows.length === 0) {
    await appendRow("workspace", tab, NOTIFICATIONS_HEADERS);
  }
  const ref = tokensRef.find((t) => t.action_key === input.action_key);
  await appendRow("workspace", tab, [
...
/**
 * Toggles one item's favourited state for the real club behind clubToken
 * (Kennedy's request, 9 Sep). Resolves club_id from the token server-side
 * rather than trusting the client-supplied value directly — same
 * ownership-verification principle already applied to submitOutreachAction
 * (5 Sep security fix), since a card's own club_id prop could otherwise be
 * tampered with client-side before this is called.
 *
 * revalidatePath covers both the Favourites page itself and the current
 * page's own layout, so a heart toggled on, say, a Players card updates
 * immediately without a full reload, and shows up on Favourites next time
 * it's visited without a stale cache.
 */
export async function toggleFavourite(
  ...
  if (!club || club.club_id !== club_id) {
    return { saved: false, error: "Could not verify this club." };
  }
  const result = await db.toggleFavourite(club.club_id, category, item_id, title, subtitle, href);
