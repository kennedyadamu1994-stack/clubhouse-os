...
import type { ActionLogRow, InboxMessage, PlayerInboxMessage, FavouriteRow } from "../types";
...
/**
 * Player Inbox read state (15 Sep) — genuinely separate table from
 * inbox_reads above, same reasoning as player_favourites' own
 * separation from favourites (see togglePlayerFavourite's own doc
 * comment): P INBOX read state is keyed by email, not club_id, and
 * reusing the real inbox_reads table's schema to accommodate both
 * would risk real CHOS inbox read data already in it.
 */
let playerInboxReadsTableReady: Promise<void> | null = null;

function ensurePlayerInboxReadsTable(): Promise<void> {
  if (!playerInboxReadsTableReady) {
    playerInboxReadsTableReady = sql`
      CREATE TABLE IF NOT EXISTS player_inbox_reads (
        email TEXT NOT NULL,
        message_id TEXT NOT NULL,
        read_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  ...
  return playerInboxReadsTableReady;
}

/**
 * Minimal rate-limit log for player Contact Us / Request a Feature
 * (15 Sep) — no ledger, no idempotency, no token_cost column, since
 * none of that applies; this table exists purely so
 * countRecentPlayerActions can enforce the same 5-per-hour real cap
 * CHOS's own free actions use, against a real timestamp history.
 */
let playerActionsTableReady: Promise<void> | null = null;

function ensurePlayerActionsTable(): Promise<void> {
  if (!playerActionsTableReady) {
    playerActionsTableReady = sql`
  ...
  return playerActionsTableReady;
}
...
/**
 * Player favourites (15 Sep) — a genuinely SEPARATE table from
 * `favourites` above, not a reuse of it. That table has club_id baked
 * directly into its primary key and Kennedy's own explicit 50-per-club
 * cap; players are identified by email, not a club_id, and changing
 * the existing table's schema to accommodate both would risk real
 * CHOS favourites data already in it. Same snapshot-at-save-time
 * design as the club version (title/subtitle/href captured once, never
 * re-fetched), same reasoning: if the underlying row is later removed
 * from the sheet, the saved favourite still shows plainly.
 *
 * Keyed by email, not a player token — matches Kennedy's own decision
 * (15 Sep) that Favourites/"For You" use the email-lookup login shown
 * in the real recommendations widget he provided, kept deliberately
 * separate from the player's own private token link (used only for
 * onboarding/welcome).
 */
let playerFavouritesTableReady: Promise<void> | null = null;

function ensurePlayerFavouritesTable(): Promise<void> {
  if (!playerFavouritesTableReady) {
    playerFavouritesTableReady = sql`
      CREATE TABLE IF NOT EXISTS player_favourites (
        ...
        href TEXT NOT NULL DEFAULT '',
        saved_at TIMESTAMPTZ NOT NULL DEFAULT now(),
        PRIMARY KEY (email, category, item_id)
      );
    `.then(() => undefined);
  }
  return playerFavouritesTableReady;
}

const PLAYER_FAVOURITES_MAX = 50;

/** Player equivalent of toggleFavourite above — same add/remove/cap behaviour, keyed by email instead of club_id. Email is normalised (trim + lowercase) at the call site, same as getPlayerByEmail, so a favourite saved as "Foo@x.com" and one saved as "foo@x.com" are always the same row. */
export async function togglePlayerFavourite(
  email: string,
  category: string,
  item_id: string,
  title: string,
  ...
  await ensurePlayerFavouritesTable();
  const existing = (await sql`
    SELECT 1 FROM player_favourites WHERE email = ${email} AND category = ${category} AND item_id = ${item_id};
  `) as { "?column?": number }[];

  if (existing.length > 0) {
    await sql`
      DELETE FROM player_favourites WHERE email = ${email} AND category = ${category} AND item_id = ${item_id};
    `;
    return { saved: false };
  }

  const countRows = (await sql`
    SELECT COUNT(*) as count FROM player_favourites WHERE email = ${email};
  `) as { count: string }[];
  const count = Number(countRows[0]?.count ?? 0);
  if (count >= PLAYER_FAVOURITES_MAX) {
    return { saved: false, error: `You can save up to ${PLAYER_FAVOURITES_MAX} favourites — remove one to add another.` };
  }

  await sql`
    INSERT INTO player_favourites (email, category, item_id, title, subtitle, href)
    VALUES (${email}, ${category}, ${item_id}, ${title}, ${subtitle}, ${href})
    ON CONFLICT (email, category, item_id) DO NOTHING;
  `;
  return { saved: true };
}

/** Player equivalent of getFavourites above. */
export async function getPlayerFavourites(email: string): Promise<FavouriteRow[]> {
  await ensurePlayerFavouritesTable();
  const rows = (await sql`
    SELECT category, item_id, title, subtitle, href, saved_at
    FROM player_favourites WHERE email = ${email}
    ORDER BY saved_at DESC;
  `) as { category: string; item_id: string; title: string; subtitle: string; href: string; saved_at: string }[];
  return rows;
}

/** Player equivalent of getFavouritedKeys above. */
export async function getPlayerFavouritedKeys(email: string): Promise<Set<string>> {
  await ensurePlayerFavouritesTable();
  const rows = (await sql`
    SELECT category, item_id FROM player_favourites WHERE email = ${email};
  `) as { category: string; item_id: string }[];
  return new Set(rows.map((r) => `${r.category}:${r.item_id}`));
}
...
export class PostgresAdapter implements DataAdapter {
  ...
  getPlayerByToken = this.local.getPlayerByToken.bind(this.local);
  getPlayerByEmail = this.local.getPlayerByEmail.bind(this.local);
  ...
  getPlayerPerks = this.local.getPlayerPerks.bind(this.local);
  getPlayerNotifications = this.local.getPlayerNotifications.bind(this.local);
  ...
  getPlayerNbrhUpdates = this.local.getPlayerNbrhUpdates.bind(this.local);
  ...
  togglePlayerFavourite = togglePlayerFavourite;
  getPlayerFavourites = getPlayerFavourites;
  getPlayerFavouritedKeys = getPlayerFavouritedKeys;
  ...
  async getPlayerInboxMessages(email: string): Promise<(PlayerInboxMessage & { read: boolean })[]> {
    const [messages, readIds] = await Promise.all([
      this.local.getPlayerInboxMessages(email), // content + a read flag we're about to discard/replace
      this.getPlayerInboxReadIds(email),
    ]);
    return messages.map((m) => ({ ...m, read: readIds.has(m.message_id) }));
  }

  async getUnreadPlayerInboxCount(email: string): Promise<number> {
  ...
  /**
   * Player Inbox read state (15 Sep) — direct equivalent of
   * getInboxReadIds/markInboxMessageRead above, keyed by email instead
   * of club_id, its own genuinely separate table (player_inbox_reads).
   */
  async getPlayerInboxReadIds(email: string): Promise<Set<string>> {
    await ensurePlayerInboxReadsTable();
    const rows = await sql`SELECT message_id FROM player_inbox_reads WHERE email = ${email};`;
    return new Set((rows as { message_id: string }[]).map((r) => r.message_id));
  }

  async markPlayerInboxMessageRead(email: string, message_id: string): Promise<void> {
    await ensurePlayerInboxReadsTable();
    await sql`
      INSERT INTO player_inbox_reads (email, message_id)
      VALUES (${email}, ${message_id})
      ON CONFLICT (email, message_id) DO NOTHING;
    `;
  }

  /**
   * Player Contact Us / Request a Feature (15 Sep, Kennedy: "the
   * Contact page in the CHOS, can you add this as an option in the
   * dropdown of the POS. With the contact us & request a feature").
   * Genuinely separate, minimal infrastructure from CHOS's own
   * submitOutreachAction — that function's real job (Actions_Log
   * ledger writes, idempotency, token balance checks, club-specific
   * revalidation) doesn't apply to a player at all, since players have
   * no token economy. This keeps ONLY the one piece that genuinely
   * matters here: a lightweight rate limit, so a public-facing form
   * can't be used to spam Kennedy's real NOTIFICATIONS sheet — same
   * 5-per-hour real limit CHOS's own free actions use, applied here
   * against a fully separate table keyed by email instead of club_id.
   */
  async countRecentPlayerActions(email: string, windowMinutes: number): Promise<number> {
    await ensurePlayerActionsTable();
    const rows = (await sql`
      SELECT COUNT(*) as count FROM player_actions_log
      WHERE email = ${email}
  ...
  async recordPlayerAction(email: string, action_key: string): Promise<void> {
    await ensurePlayerActionsTable();
    await sql`
      INSERT INTO player_actions_log (email, action_key)
      VALUES (${email}, ${action_key});
    `;
  }
