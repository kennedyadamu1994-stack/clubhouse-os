...
export function generateClubToken(prefix: string): string {
  ...
  // Rejection sampling avoids modulo bias: 256 isn't evenly divisible by
  // 62 (alphabet.length), so a plain `byte % 62` would make positions
  // 0-7 come up slightly more often than 8-61 — a real, if small,
  // weakening of a value that functions as an access credential. The
  // largest multiple of 62 that fits in a byte is 62*4=248, so any byte
  // >= 248 is discarded and redrawn rather than reused.
