...
/**
 * OPPORTUNITIES' real "Paid / Unpaid" column is a two-way label, not a
 * Y/N flag — toBool() would read "Paid" as false (it only matches
 * y/yes/true/1), so a dedicated reader is needed rather than reusing
 * toBool() the way DBS Required (a genuine Y/N column) correctly does.
 * Defaults to false (unpaid) on blank/unrecognised, matching toBool's
 * own fail-safe default rather than assuming paid.
 */
export function isPaidOpportunity(v: string | undefined): boolean {
  if (!v) return false;
  return /^paid$/i.test(v.trim());
}
...
/**
 * Month names → 1-12, both full ("October") and 3-letter abbreviated
 * ("Oct"), lowercased for a case-insensitive match. Kennedy's real
 * CALENDAR tab (confirmed via screenshot, 14 Sep) holds the full English
 * name in the Month column, not a number — parseMonth() below tries a
 * plain number first (in case the column is ever switched to numeric)
 * and falls back to this map, so either format works without more code
 * changes if the sheet changes again.
 */
...
function parseMonth(month: string | undefined): number | null {
  if (!month) return null;
  const trimmed = month.trim();
  const asNumber = Number(trimmed);
  if (!Number.isNaN(asNumber) && trimmed !== "") return asNumber;
  const named = MONTH_NAMES[trimmed.toLowerCase()];
  return named ?? null;
}
 ...
 * Month accepts either a number or an English month name (see
 * parseMonth() above) — the real sheet uses the name, e.g. "21 / October /
 * 2026". Falls back to today's date on unparseable input rather than
 * throwing — one bad row in Kennedy's hand-maintained Calendar tab
 * shouldn't break the whole Calendar page (architecture.md § Failure).
 */
export function combineCalendarDate(
  ...
  const y = Number(year);
  if (!d || !m || !y || Number.isNaN(d) || Number.isNaN(y)) {
