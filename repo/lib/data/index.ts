...
export interface SubmitActionInput {
  ...
  /**
   * The club's real plan allocation (7 Sep fix) — without this,
   * submitAction's own internal balance check fell back to Config's
   * generic allocation_default (5), never the club's real per-tier
   * allocation (10/18/25/40), which getTokenBalance/resetTokenBalance
   * were already correctly given via this same parameter name. A club
   * on any plan above the 5-token default would see their header show
   * a real, correct, much higher balance while every paid action kept
   * getting rejected the moment their real spend crossed 5 tokens
   * total — exactly the "header says 21/25, but this got rejected as
   * unaffordable" symptom Kennedy reported. Optional so a caller that
   * genuinely has no plan context (or LocalAdapter, which computes its
   * own allocation) can omit it and fall back to the old default.
   */
...
export interface DataAdapter {
  ...
  /** Player equivalent of getPerks above — reads a genuinely separate real sheet (P PERKS); see PlayerPerk's own doc comment (lib/types.ts) for why. */
  getPlayerPerks(): Promise<PlayerPerk[]>;
  ...
  /**
   * POS's own "From The NBRH" feed (17 Sep, Kennedy) — a genuinely
   * separate real tab, "P FROM THE NBRH" on CHOS Workspace, same four
   * columns as "FROM THE NBRH" (Date, Note, URL, Image_URL) so players
   * see content curated for them rather than CHOS's own club-facing
   * updates. Platform-wide, not per-club — same shape as getNbrhUpdates.
   */
  getPlayerNbrhUpdates(): Promise<NbrhUpdate[]>;
  getFaq(): Promise<Faq[]>;
  ...
  /**
   * Player equivalent of the three methods above, keyed by email
   * instead of club_id — genuinely separate real sheet (P INBOX) and a
   * genuinely separate Postgres read-state table (player_inbox_reads),
   * same reasoning as togglePlayerFavourite's own separation from
   * toggleFavourite (see that method's own doc comment): reusing the
   * real INBOX/inbox_reads infrastructure directly would mean changing
   * their schema to accommodate an email in place of a club_id, risking
   * real CHOS inbox data already in it.
   */
  getPlayerInboxMessages(email: string): Promise<(PlayerInboxMessage & { read: boolean })[]>;
  getUnreadPlayerInboxCount(email: string): Promise<number>;
  markPlayerInboxMessageRead(email: string, message_id: string): Promise<void>;
  /**
   * Rate-limit support for player Contact Us / Request a Feature
   * (15 Sep) — see the real Postgres implementation's own doc comment
   * (lib/data/postgres.ts) for the full reasoning: a genuinely minimal,
   * separate table from CHOS's own actions_log ledger.
   */
  countRecentPlayerActions(email: string, windowMinutes: number): Promise<number>;
  recordPlayerAction(email: string, action_key: string): Promise<void>;
  /** Reads the real NOTIFICATIONS sheet, filtered to player-originated rows — see the real SheetsAdapter implementation's own doc comment for the full reasoning. */
  getPlayerNotifications(): Promise<{ timestamp: string; from: string; action: string; notes: string }[]>;
  /**
  ...
  /**
   * Favourites (Kennedy's request, 9 Sep). toggleFavourite adds or
   * removes one item, capturing a snapshot of its display details at
   * save time (title/subtitle/href) — see the real implementation in
   * lib/data/postgres.ts for the full reasoning, including why this is
   * a snapshot rather than a live reference and why the 50-item cap
   * only applies on the add path.
   */
  toggleFavourite(
    club_id: string,
    category: string,
  ...
  getFavourites(club_id: string): Promise<FavouriteRow[]>;
  getFavouritedKeys(club_id: string): Promise<Set<string>>;
  /**
   * Player favourites (15 Sep) — same shape as toggleFavourite/
   * getFavourites/getFavouritedKeys above, but keyed by email rather
   * than club_id, and stored in a genuinely separate table (see the
   * real Postgres implementation, lib/data/postgres.ts, for why: that
   * table's own primary key structure and 50-per-club cap are specific
   * to clubs, and reusing it would risk real existing CHOS data).
   * Powers POS's own email-lookup Favourites/"For You" (Kennedy's
   * decision, 15 Sep — a separate identity from the player's own
   * private token link, used only for onboarding/welcome).
   */
  ...
  getPlayerFavourites(email: string): Promise<FavouriteRow[]>;
  getPlayerFavouritedKeys(email: string): Promise<Set<string>>;
  /**
   ...
   */
  getPlayersUnfiltered(forceFresh?: boolean): Promise<Player[]>;
  /**
   * A SECOND, narrower exception to the never-show-player-names rule
   * (POS, 15 Sep) — deliberately its own method rather than a reuse of
   * getPlayersUnfiltered above, even though both return an ungated
   * Player. getPlayersUnfiltered is explicitly documented as
   * admin-only, never reachable from a real club or player route; this
   * one is the opposite — it's ONLY ever reachable from a player's own
   * POS pages (/players/[playerToken]/...), the one legitimate
   * player-facing case where showing a real name is correct: a player
   * looking at their own welcome page should see their own name, the
   * same way a club sees its own name on its dashboard. Looks up by
   * the real token, not by scanning + exposing the whole player list,
   * so the only name ever returned is the one the caller already
   * proved they hold the private link for.
   */
  getPlayerByToken(token: string, forceFresh?: boolean): Promise<Player | null>;
  /** Email-based lookup for POS's Favourites/"For You" login (15 Sep) — see getPlayerByToken's own doc comment for why real email is intentionally returned here, and Player.email's doc comment (lib/types.ts) for the full gating story. */
  getPlayerByEmail(email: string, forceFresh?: boolean): Promise<Player | null>;
  /**
...
export function gatePlayer(p: Player): Player {
  ...
  // email gated the same unconditional way (15 Sep) — added for POS's
  // own email-lookup login, never meant to be visible to a club.
  return { ...p, name: null, email: null };
}
...
export class LocalAdapter implements DataAdapter {
  ...
  async getPlayersUnfiltered(_forceFresh = false) {
    return readJson<Player[]>("players.json"); // no gatePlayer() — see interface doc comment; forceFresh is a no-op here, there's no live-sheet cache to bypass against a plain local JSON fixture
  }
  async getPlayerByToken(token: string, _forceFresh = false) {
    const players = await readJson<Player[]>("players.json");
    return players.find((p) => p.player_id === token) ?? null;
  }
  async getPlayerByEmail(email: string, _forceFresh = false) {
  ...
  async getPlayerPerks() {
    return readJson<PlayerPerk[]>("player_perks.json");
  }
  ...
  async getPlayerNbrhUpdates() {
    const rows = await readJson<NbrhUpdate[]>("player_nbrh_updates.json");
    return rows.sort((a, b) => b.date.localeCompare(a.date));
  }
  async getFaq() {
  ...
  async getPlayerInboxMessages(email: string) {
    const [messages, reads] = await Promise.all([
      readJson<PlayerInboxMessage[]>("player_inbox_messages.json"),
      readJson<{ email: string; message_id: string; read_at: string }[]>("player_inbox_read_state.json"),
    ]);
    const target = email.trim().toLowerCase();
    const readIds = new Set(
      reads.filter((r) => r.email.trim().toLowerCase() === target).map((r) => r.message_id),
    );
    return messages
  ...
  async getUnreadPlayerInboxCount(email: string) {
    const all = await this.getPlayerInboxMessages(email);
    return all.filter((m) => !m.read).length;
  }
  async markPlayerInboxMessageRead(email: string, message_id: string) {
    const file = path.join(DATA_DIR, "player_inbox_read_state.json");
    const reads = await readJson<{ email: string; message_id: string; read_at: string }[]>("player_inbox_read_state.json");
    const target = email.trim().toLowerCase();
    if (reads.some((r) => r.email.trim().toLowerCase() === target && r.message_id === message_id)) return;
    reads.push({ email: target, message_id, read_at: new Date().toISOString() });
    await fs.writeFile(file, JSON.stringify(reads, null, 2));
  }
  async countRecentPlayerActions(email: string, windowMinutes: number) {
    const rows = await readJson<{ email: string; action_key: string; created_at: string }[]>("player_actions_log.json");
    const target = email.trim().toLowerCase();
    const cutoff = Date.now() - windowMinutes * 60_000;
    return rows.filter((r) => r.email.trim().toLowerCase() === target && new Date(r.created_at).getTime() > cutoff).length;
  }
  async recordPlayerAction(email: string, action_key: string) {
    const file = path.join(DATA_DIR, "player_actions_log.json");
    const rows = await readJson<{ email: string; action_key: string; created_at: string }[]>("player_actions_log.json");
    rows.push({ email: email.trim().toLowerCase(), action_key, created_at: new Date().toISOString() });
    await fs.writeFile(file, JSON.stringify(rows, null, 2));
  }
  async getPlayerNotifications() {
    return readJson<{ timestamp: string; from: string; action: string; notes: string }[]>("player_notifications.json");
  }
  ...
  async toggleFavourite(
    ...
    const file = path.join(DATA_DIR, "favourites.json");
    const all = await readJson<(FavouriteRow & { club_id: string })[]>("favourites.json");
    const idx = all.findIndex((f) => f.club_id === club_id && f.category === category && f.item_id === item_id);
    if (idx >= 0) {
      all.splice(idx, 1);
      await fs.writeFile(file, JSON.stringify(all, null, 2));
      return { saved: false };
    }
    const clubCount = all.filter((f) => f.club_id === club_id).length;
    if (clubCount >= 50) {
      return { saved: false, error: "You can save up to 50 favourites — remove one to add another." };
    }
    all.push({ club_id, category, item_id, title, subtitle, href, saved_at: new Date().toISOString() });
    await fs.writeFile(file, JSON.stringify(all, null, 2));
    return { saved: true };
  }
  async getFavourites(club_id: string): Promise<FavouriteRow[]> {
    const all = await readJson<(FavouriteRow & { club_id: string })[]>("favourites.json");
    return all
      .filter((f) => f.club_id === club_id)
      .sort((a, b) => b.saved_at.localeCompare(a.saved_at))
      .map(({ club_id: _club_id, ...rest }) => rest);
  }
  async getFavouritedKeys(club_id: string): Promise<Set<string>> {
    const all = await readJson<(FavouriteRow & { club_id: string })[]>("favourites.json");
    return new Set(all.filter((f) => f.club_id === club_id).map((f) => `${f.category}:${f.item_id}`));
  }
  async togglePlayerFavourite(
    email: string,
    category: string,
    item_id: string,
    title: string,
    ...
    const file = path.join(DATA_DIR, "player_favourites.json");
    const all = await readJson<(FavouriteRow & { email: string })[]>("player_favourites.json");
    const idx = all.findIndex((f) => f.email === email && f.category === category && f.item_id === item_id);
    if (idx >= 0) {
      all.splice(idx, 1);
      await fs.writeFile(file, JSON.stringify(all, null, 2));
      return { saved: false };
    }
    const emailCount = all.filter((f) => f.email === email).length;
    if (emailCount >= 50) {
      return { saved: false, error: "You can save up to 50 favourites — remove one to add another." };
    }
    all.push({ email, category, item_id, title, subtitle, href, saved_at: new Date().toISOString() });
    await fs.writeFile(file, JSON.stringify(all, null, 2));
    return { saved: true };
  }
  async getPlayerFavourites(email: string): Promise<FavouriteRow[]> {
    const all = await readJson<(FavouriteRow & { email: string })[]>("player_favourites.json");
    return all
  ...
  async getPlayerFavouritedKeys(email: string): Promise<Set<string>> {
    const all = await readJson<(FavouriteRow & { email: string })[]>("player_favourites.json");
    return new Set(all.filter((f) => f.email === email).map((f) => `${f.category}:${f.item_id}`));
  }
