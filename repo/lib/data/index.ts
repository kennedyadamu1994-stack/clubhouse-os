...
export interface DataAdapter {
  ...
  /**
   * Real HEADER tab, CHOS Workspace (Kennedy, 4 Sep) — the header
   * carousel's real image source, replacing the hardcoded
   * DEMO_CAROUSEL_IMAGES placeholder. Platform-wide, not per-club. Sorted
   * by "Header Order" ascending; a missing/empty tab returns [] rather
   * than throwing (see getHeaderImages' own implementation), so the
   * carousel component's existing empty-array handling (renders nothing)
   * covers that case with no extra work.
   */
