...
export interface DataAdapter {
   ...
   * until the real "SOCIAL VENUES" worksheet exists; see SocialVenue's
...
export class LocalAdapter implements DataAdapter {
  ...
  async getSocialVenues() {
    // Placeholder — no real "SOCIAL VENUES" worksheet exists yet (Kennedy,
    // 27 Aug). Same seed-data-until-a-real-sheet-exists pattern as
    // Brands/Influencers before their reconciliation.
  ...
  async getInboxMessages(club_id: string) {
    ...
    return messages
      ...
      // Chronological (Kennedy's request, 27 Aug) — oldest first, matching
      // how a real inbox/thread reads top-to-bottom.
