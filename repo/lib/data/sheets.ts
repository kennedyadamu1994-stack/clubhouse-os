 ...
 *
 * clearAllActions() (admin "clear all data logs" button, app/admin/requests): deletes EVERY row
 * in the Actions_Log tab/sheet. A full wipe, not a correction — the second deliberate exception
 * to append-only. Irreversible; also resets every club's computed balance to full allocation,
 * since balance derives from the ledger. Confirmation UI lives in the calling component, not here.
 *
 ...
 *     source_type, summary, why_it_matters, url, published_at, trending_score, interest_score,
 *     content_angle {title, medium}, thumbnail_url), sorted newest-first. Read-only external
 *     links, no write path. Lives under Workspace. Every club sees the same list for now; the
 *     "why_it_matters" copy is written to reference club goals generically (real per-club
 *     tailoring would be a later enhancement, not needed for the pilot). Redesigned 19 Aug per
 *     Kennedy's reference "Trending Topics Desk" component — trending_score/interest_score and
 *     content_angle are currently hand-set dummy values per story; Kennedy's stated intent is
 *     for this whole tab to eventually be the live source (a scoring/content-angle pipeline
 *     writing into the sheet), not just a read target for the same static fields as before.
 */
export {};
