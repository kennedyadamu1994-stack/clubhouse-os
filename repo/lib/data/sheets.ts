...
export class SheetsAdapter implements DataAdapter {
   ...
   * player submission doesn't have). Reads the real NOTIFICATIONS sheet
   * back — genuinely written to already by notifyKennedyOfAction() for
   * every player submission, but never read back anywhere until now.
   * Filtered to rows whose "action label" starts with "player_" — every
   * player action_key (player_contact_us, player_feature_request) has
   * no matching tokensRef entry, so notifyKennedyOfAction's own
   * `ref?.label ?? input.action_key` fallback writes the literal key
   * string, which is what this filters on; every real club submission
   * always has a real tokensRef label instead (e.g. "Contact us"), so
   * there's no genuine risk of a false match either direction.
   */
  async getPlayerNotifications(): Promise<{ timestamp: string; from: string; action: string; notes: string }[]> {
    const rows = await readTabSafe("workspace", "NOTIFICATIONS");
    if (rows.length === 0) return [];
    const objects = rowsToObjects(rows);
    return objects
      .filter((r) => (r["action label"] ?? "").startsWith("player_"))
      .map((r) => ({
        timestamp: r["timestamp"] ?? "",
        from: r["club name"] ?? "", // real column name on this sheet is "club name" even for a player row — see notifyKennedyOfAction's own input shape (clubName), reused as-is rather than renamed, since renaming it would mean either a second sheet or a schema migration for one column's label
