...
export class SheetsAdapter implements DataAdapter {
  ...
  private rowToClub(r: Record<string, string | undefined>): Club {
    return {
      ...
      priorities: toList(r["Priorities"]),
      goals: r["Goals"] ?? "",
  ...
  async setClubPriorities(club_id: string, priorities: string[], goals: string): Promise<void> {
    // Writes to DASHBOARD (X)'s Priorities/Goals columns — added 20 Aug per
    // Kennedy's decision to add real columns rather than leave this
    // unimplemented. Finds which row this club_id is on, then writes only
    // the Priorities/Goals cells for that row.
    const { google } = await import("googleapis");
    const rows = await readTab("master", "DASHBOARD (X)");
    const headers = rows[0].map((h) => h.trim());
    const idCol = headers.indexOf("Club ID");
    const priCol = headers.indexOf("Priorities");
    const goalsCol = headers.indexOf("Goals");
    const rowIndex = rows.slice(1).findIndex((r) => r[idCol] === club_id);
    if (rowIndex === -1) throw new Error("Club not found");
    if (priCol === -1 || goalsCol === -1) {
      throw new Error(
        "DASHBOARD (X) is missing the Priorities/Goals columns — add them before saving priorities.",
      );
    }
    ...
    await client.spreadsheets.values.update({
      spreadsheetId: process.env.GOOGLE_SHEET_ID_MASTER,
      range: `DASHBOARD (X)!${colLetter(priCol)}${sheetRow}:${colLetter(goalsCol)}${sheetRow}`,
      valueInputOption: "RAW",
      requestBody: { values: [[priorities.join(", "), goals]] },
    });
  }
  ...
  async getPlayers(): Promise<Player[]> {
    ...
    // Deliberately NOT reading: Email, Date of Birth, Gender, Disability
    // Status, Phone, Height, Position, WhatsApp Consent, Motivations, Age —
    // real columns on NEIGHBOURS (O) with sensitive personal data this app
    // has no use for and must never surface (20 Aug reconciliation).
    return rows.map((r, i) => ({
      ...
      consent_share_name: false, // irrelevant now — gatePlayer() in index.ts always nulls the name regardless (Kennedy, 20 Aug)
      name: null, // NEVER populated from the sheet — see gatePlayer(), the actual enforcement point
    }));
  }
  ...
  async getClubsDirectory(): Promise<ClubDirectoryEntry[]> {
    // CLUBS (O) is out of scope per Kennedy's 20 Aug confirmation (same
    // bucket as SESSIONS/LEAGUES/VENUES/EVENTS — a different, already-live
    // sheet is the real source). Falls back to local seed data so the
    // Clubs directory subsection doesn't go empty/break.
    return this.local.getClubsDirectory();
  }
