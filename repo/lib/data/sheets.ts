...
export class SheetsAdapter implements DataAdapter {
  ...
  async getInboxMessages(club_id: string): Promise<(InboxMessage & { read: boolean })[]> {
    ...
    // Message, URL — routes by the club's NAME, not a club_id column (one
    // doesn't exist on this sheet). Resolves club_id -> name once here so
    // every existing caller (the Inbox page, the header's unread count)
    // keeps calling this the same way as every other per-club method.
    // Exact match, case-insensitive/trimmed — see InboxMessage's doc
    // comment in lib/types.ts for the real fragility this introduces (a
    // renamed club or a sheet typo silently drops messages, with no way
    // to tell "no messages" apart from "the name didn't match").
    ...
    const messages: InboxMessage[] = rows
      .filter((r) => myName && r["Club"]?.trim().toLowerCase() === myName)
      .map((r, i) => ({
  ...
  async getHeaderImages(): Promise<HeaderImage[]> {
    // Real "HEADER" tab, CHOS Workspace (Kennedy, 4 Sep, confirmed via
    // screenshot) — columns: Header Order ("#1" style, col A), Image URL
    // (col B), URL (col C, may be blank). This is the carousel's real
    // image source, replacing the hardcoded DEMO_CAROUSEL_IMAGES
    // placeholder array that stood in after an earlier hotlinked-image
    // version broke in production and was never reconnected to a real
    // sheet-backed source. readTabSafe rather than readTab — same
    // caution as KIT/INBOX (see that function's own doc comment): a
    // missing/not-yet-created HEADER tab must never take down every page
    // on the site, since this renders via the shared layout on Overview.
    const rows = rowsToObjects(await readTabSafe("workspace", "HEADER"));
    return rows
      .filter((r) => r["Image URL"])
      .map((r, i) => {
        // "Header Order" is "#1" style text, not a plain number — strip
        // any non-digit characters before parsing. Falls back to this
        // row's array position (so it still sorts somewhere sensible)
        // if the cell is genuinely blank or unparseable, rather than
        // producing NaN and silently breaking the sort.
        const parsedOrder = parseInt((r["Header Order"] ?? "").replace(/\D/g, ""), 10);
        return {
          order: isNaN(parsedOrder) ? i : parsedOrder,
