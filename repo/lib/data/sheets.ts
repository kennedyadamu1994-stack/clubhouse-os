...
import { toBool, toList, toDashList, toNumberOrNull, toNumber, combineKpiPairs, combineCalendarDate, readSponsored, filterContextualTags, isPaidOpportunity } from "./sheets/parse";
import { parseRecommendedForClubIds } from "../scoring";
 ...
 */
async function readTabSafe(source: SheetSource, tabName: string, forceFresh = false): Promise<string[][]> {
  try {
    return await readTab(source, tabName, forceFresh);
  } catch {
    return [];
  }
}
...
export class SheetsAdapter implements DataAdapter {
  ...
  /**
   * Resolves the club's real plan allocation before delegating to the
   * real ledger (7 Sep fix) — previously a plain delegation with no
   * allocation supplied at all, meaning every one of submitAction's own
   * internal balance checks (the actual hard-cap enforcement) silently
   * used the generic Config default rather than this club's real
   * per-tier allocation. Same lookup getTokenBalance already does
   * correctly, just applied here too so both paths agree.
   */
  submitAction = async (input: SubmitActionInput): Promise<SubmitActionResult> => {
    const [clubs, plans] = await Promise.all([this.getAllClubs(), this.getPlans()]);
    const club = clubs.find((c) => c.club_id === input.club_id);
    const plan = club ? plans.find((p) => p.tier === club.plan_tier) : undefined;
    return this.ledger.submitAction({ ...input, allocationOverride: plan?.tokens });
  };
  ...
  toggleFavourite = this.ledger.toggleFavourite.bind(this.ledger);
  getFavourites = this.ledger.getFavourites.bind(this.ledger);
  getFavouritedKeys = this.ledger.getFavouritedKeys.bind(this.ledger);
  togglePlayerFavourite = this.ledger.togglePlayerFavourite.bind(this.ledger);
  getPlayerFavourites = this.ledger.getPlayerFavourites.bind(this.ledger);
  getPlayerFavouritedKeys = this.ledger.getPlayerFavouritedKeys.bind(this.ledger);
  /**
  ...
  /**
   * Player Inbox (15 Sep) — real P INBOX tab, CHOS Workspace workbook,
   * a genuinely separate sheet from INBOX above. Real columns confirmed
   * directly by Kennedy (15 Sep): Entry ID, Date, Club, Title, Message,
   * URL, Email. Routes by the real Email column directly, exact match
   * case-insensitive/trimmed (no name-fallback matching needed the way
   * INBOX's own club_id required — P INBOX already has a genuinely
   * stable Email column from the start). Read state comes from the
   * ledger's own separate player_inbox_reads table, not from mapping a
   * read flag off this read directly — same split as INBOX's own
   * club-scoped version, same reasoning (CLAUDE.md rule 9: hand-
   * maintained sheet content stays separate from click-driven read
   * state, which a Sheet is a poor fit for).
   */
  async getPlayerInboxMessages(email: string): Promise<(PlayerInboxMessage & { read: boolean })[]> {
    const target = email.trim().toLowerCase();
    const rows = rowsToObjects(await readTabSafe("workspace", "P INBOX"));
    const messages: PlayerInboxMessage[] = rows
      .filter((r) => (r["Email"] ?? "").trim().toLowerCase() === target)
      .map((r, i) => ({
        message_id: r["Entry ID"] ?? `pinbox_${i}`,
        email: target,
        sent_at: r["Date"] ?? new Date().toISOString(),
        title: r["Title"] || (r["Message"] ?? "").slice(0, 60).trimEnd(),
        message: r["Message"] ?? "",
      ...
      .sort((a, b) => b.sent_at.localeCompare(a.sent_at)); // newest first, same as INBOX's own real ordering

    const readIds = await this.ledger.getPlayerInboxReadIds(target);
    return messages.map((m) => ({ ...m, read: readIds.has(m.message_id) }));
  }
  getUnreadPlayerInboxCount = async (email: string): Promise<number> => {
    const all = await this.getPlayerInboxMessages(email);
    return all.filter((m) => !m.read).length;
  };
  markPlayerInboxMessageRead = this.ledger.markPlayerInboxMessageRead.bind(this.ledger);
  countRecentPlayerActions = this.ledger.countRecentPlayerActions.bind(this.ledger);
  recordPlayerAction = this.ledger.recordPlayerAction.bind(this.ledger);

  /**
   * Player Contact Us / Request a Feature submissions (16 Sep, Kennedy:
   * "connect contact page to datalog" — resolved as its own separate,
   * simple list, not merged into CHOS's real admin requests page, since
   * that page is built entirely around club token/status workflows a
   * player submission doesn't have).
   *
   * Reads the real P NOTIFICATIONS sheet (18 Sep — Kennedy created this
   * as its own dedicated tab in the same workspace workbook, "exclusively
   * for players", and notifyKennedyOfAction()'s player call site now
   * writes there instead of sharing NOTIFICATIONS with club rows — see
   * lib/actions.ts and lib/players/actions.ts). Previously this read
   * NOTIFICATIONS filtered to rows whose "action label" started with
   * "player_"; that filter is gone since every row in this tab is now a
   * player row by construction, not by label-matching. Historical player
   * rows written before this split live in the old NOTIFICATIONS tab and
   * won't appear here — Kennedy owns the sheets directly, so moving that
   * history across (or not) is his call, not something this read decides.
   *
   * forceFresh: true (19 Sep fix, Kennedy: "It's not showing up in the
   * data log page") — this is an admin-only, low-traffic page someone
   * checks right after a submission comes in, so the normal 5-minute
   * readTab cache is exactly wrong here: without this, a submission
   * landing just after this tab was last read (by anyone, anywhere in
   * the app) would stay invisible on this page for up to 5 real minutes
   * even though it's genuinely already in the sheet.
   */
  async getPlayerNotifications(): Promise<{ timestamp: string; from: string; action: string; notes: string }[]> {
    const rows = await readTabSafe("workspace", "P NOTIFICATIONS", true);
    if (rows.length === 0) return [];
    const objects = rowsToObjects(rows);
    return objects
      .map((r) => ({
        timestamp: r["timestamp"] ?? "",
        from: r["club name"] ?? "", // real column name on this sheet is "club name" even for a player row — see notifyKennedyOfAction's own input shape (clubName), reused as-is rather than renamed, since renaming it would mean a schema migration for one column's label
        action: r["action label"] === "player_contact_us" ? "Contact Us" : r["action label"] === "player_feature_request" ? "Request a Feature" : (r["action label"] ?? ""),
        notes: r["notes"] ?? "",
      }))
  ...
  private rowToPlayer(r: Record<string, string | undefined>, i: number): Player {
    return {
      ...
      email: r["Email"] || null, // real value here; gatePlayer() always nulls it, same treatment as name
      ...
      recommended_for_club_ids: parseRecommendedForClubIds(r["U notes"]),
    };
  }
  ...
  async getPlayersUnfiltered(forceFresh = false): Promise<Player[]> {
    ...
    const rows = rowsToObjects(await readTab("master", "NEIGHBOURS (O)", forceFresh));
    return rows.map((r, i) => this.rowToPlayer(r, i));
  }

  /**
   * Real name preserved — see interface doc comment (index.ts) for why
   * this is a deliberate, separate exception from getPlayers()'s
   * always-null-name gate, scoped to a player looking up their own row
   * by their own token. Matches on the real "record_id" column, which
   * IS the player's token (see submitOnboarding, lib/players/actions.ts
   * — token doubles as record_id at row-creation time, never derived
   * from their name).
   *
   * forceFresh (17 Sep) — see readTab's own doc comment (lib/data/
   * sheets/client.ts) for the real race this guards against: a fresh
   * signup's row may not have propagated through NEIGHBOURS (O)'s own
   * array formula yet, and this app's 5-minute Sheets cache could
   * otherwise serve a stale pre-signup copy for real minutes even after
   * the sheet itself has updated.
   */
  async getPlayerByToken(token: string, forceFresh = false): Promise<Player | null> {
    const players = await this.getPlayersUnfiltered(forceFresh);
    return players.find((p) => p.player_id === token) ?? null;
  }

  /**
   * Real email preserved — same deliberate exception from getPlayers()'s
   * always-null-email gate as getPlayerByToken's own name exception (see
   * that method's doc comment, and Player.email's own doc comment in
   * lib/types.ts). Used by POS's email-lookup login for Favourites/"For
   * You" (Kennedy, 15 Sep) — matched case-insensitively and trimmed,
   * since a person may type their email with different casing/whitespace
   * than however it was originally entered at onboarding.
   *
   * forceFresh (17 Sep) — see getPlayerByToken's own doc comment above
   * for the real race condition this guards against.
   */
  async getPlayerByEmail(email: string, forceFresh = false): Promise<Player | null> {
    const target = email.trim().toLowerCase();
    if (!target) return null;
    const players = await this.getPlayersUnfiltered(forceFresh);
    return players.find((p) => (p.email ?? "").trim().toLowerCase() === target) ?? null;
  }
  ...
  private rowToPersonFromCreator(r: Record<string, string | undefined>, i: number): Person {
    return {
      ...
      bio: null, // CONTENT CREATORS has no documented equivalent column — see Person.bio's own doc comment
      recommended_for_club_ids: parseRecommendedForClubIds(r["U notes"]),
    };
  }
  ...
  private rowToBrand(r: Record<string, string | undefined>, i: number): Brand {
    return {
      ...
      // Real "source_url" column, BRANDS sheet (confirmed by Kennedy,
      // 7 Sep — this app's Brand.website field). NOTE: on Supplier and
      // SocialVenue, a column also called "source_url" means something
      // different entirely (internal provenance: where the listing was
      // sourced from, never shown to a club) — the same column name is
      // genuinely used for two different real things on different
      // sheets, this is not a copy-paste mistake. Previously hardcoded
      // to "" with a comment claiming this column didn't exist, which
      // meant the black "Visit website" button already built into the
      // Brands page (app/dashboard/[clubToken]/outreach/brands/page.tsx)
      // silently never rendered, for any brand, from either source.
      ...
      tags: filterContextualTags(toList(r["tags"])),
      credibility_score: toNumberOrNull(r["credibility_score"]),
      // Real columns, BRANDS sheet only (7 Sep, Kennedy: "more info in
      // the expanded panel... Marketing Budget Signal & CSR/ESG Sport
      // Mandate"). Confirmed NOT present on the separate BUSINESSES
      // sheet, so rowToBrandFromBusiness below correctly leaves these
      // null rather than guessing at an equivalent column there.
      marketing_budget_signal: r["Marketing Budget Signal"] || null,
      csr_esg_sport_mandate: r["CSR/ESG Sport Mandate"] || null,
      recommended_for_club_ids: parseRecommendedForClubIds(r["U notes"]),
    };
  }
  ...
  private rowToBrandFromBusiness(r: Record<string, string | undefined>, i: number): Brand {
    return {
      ...
      // Real "source_url" column, BUSINESSES sheet — same fix and same
      // reasoning as rowToBrand's own "source_url" doc comment above.
      website: r["source_url"] || "",
      ...
      tags: filterContextualTags(toList(r["tags"])),
      credibility_score: toNumberOrNull(r["credibility_score"]),
      // Confirmed by Kennedy (7 Sep): BUSINESSES has no equivalent to
      // BRANDS' "Marketing Budget Signal"/"CSR/ESG Sport Mandate"
      // columns — null here is a genuine, confirmed absence, not a
      // guess or an oversight.
      marketing_budget_signal: null,
      csr_esg_sport_mandate: null,
      recommended_for_club_ids: parseRecommendedForClubIds(r["U notes"]),
    };
  }
  ...
  async getSuppliers(): Promise<Supplier[]> {
    ...
    return rows.map((r, i) => ({
      ...
      tags: toList(r["tags"]),
      // Real "U notes" column, parsed into the manual recommendation
      // list (7 Sep) — previously read into internal_notes/private
      // notes, a purpose Kennedy confirmed he doesn't actually use.
      recommended_for_club_ids: parseRecommendedForClubIds(r["U notes"]),
  ...
  /**
   * OPPORTUNITIES tab, master sheet — sports/exercise jobs, volunteering,
   * and placements for POS's Jobs page (15 Sep). Real tab, read once
   * before, stopped 9 Sep when CHOS's own Outreach → Opportunities
   * subsection was removed; re-wired here for POS, a separate surface.
   * Read-only: no in-app club-submission flow exists on the real sheet
   * (no submitted_by_club_id column), so entries are whatever Kennedy
   * adds directly. PREMIUM/CORE deliberately not read — every
   * opportunity shows to every player regardless (Kennedy, 15 Sep).
   */
  async getOpportunities(): Promise<Opportunity[]> {
    const rows = rowsToObjects(await readTab("master", "OPPORTUNITIES"));
    return rows.map((r, i) => ({
      opportunity_id: r["Opportunity ID"] ?? r["record_id"] ?? `opp_${i}`,
      created_at: r["date_added"] ?? r["Date Added"] ?? new Date().toISOString().slice(0, 10),
      sponsored: readSponsored(r),
      title: r["Title"] ?? "",
      type: r["Type"] ?? "",
      sports: toList(r["Sport(s)"]),
      provider: r["Organisation / Club"] ?? "",
      area: r["Location (Borough)"] ?? "",
      frequency: r["Frequency"] ?? "",
      description: r["Description"] ?? "",
      requirements: r["Requirements / Qualifications"] ?? "",
      experience_level: r["Experience Level"] ?? "",
      dbs_required: toBool(r["DBS Required"]),
      paid: isPaidOpportunity(r["Paid / Unpaid"]),
      start_date: r["Start Date"] ?? "",
      closing_date: r["Deadline"] ?? "",
      hours_per_week: r["Hours Per Week"] ?? "",
      contact_name: r["Contact Name"] ?? "",
      contact_email: r["Contact Email"] ?? "",
      apply_url: r["Application Link / URL"] ?? "",
      status: r["Status"] ?? "",
      notes: r["Notes"] ?? "",
      image_url: r["Image URL"] || null,
      tags: filterContextualTags(toList(r["tags"])),
  ...
  async getNbrhUpdates(): Promise<NbrhUpdate[]> {
    ...
    // URL (col C, may be blank), Image_URL (added 16 Sep, may be blank —
    // not every update needs an image). This was never actually wired up
    // before (Kennedy's report, 25 Aug: the note "isn't connecting to the
    // sheet" — true, community_note on Club was hardcoded to an empty
    // string and this method didn't exist at all). Skips rows with no
    // Note text rather than showing an empty feed entry.
  ...
  async getPlayerNbrhUpdates(): Promise<NbrhUpdate[]> {
    // "P FROM THE NBRH" tab, CHOS Workspace (17 Sep, Kennedy) — a
    // genuinely separate tab from "FROM THE NBRH" above, same four
    // real columns (Date, Note, URL, Image_URL), so POS shows content
    // curated for players rather than CHOS's own club-facing feed.
    const rows = rowsToObjects(await readTab("workspace", "P FROM THE NBRH"));
    return rows
      .filter((r) => r["Note"])
      .map((r, i) => ({
        update_id: `pnbrh_${i}`,
  ...
  /**
   * Player perks (15 Sep) — real P PERKS tab, CHOS Workspace workbook, a
   * genuinely separate sheet from PERKS above. Column names confirmed
   * directly by Kennedy (15 Sep, real headers): Perk Entry, Perk Type,
   * Perk Owner, Title, Perk Offer, Description, Perk Code, Perk URL —
   * identical to PERKS' own real columns, as the reasoned assumption
   * this mapping originally shipped with expected (same workbook, same
   * team, same real purpose). No plan-tier column on either sheet —
   * every perk shown to every player, nothing locked, matching
   * Kennedy's confirmation (15 Sep).
   */
  async getPlayerPerks(): Promise<PlayerPerk[]> {
    const rows = rowsToObjects(await readTab("workspace", "P PERKS"));
    return rows.map((r, i) => ({
      perk_id: r["Perk Entry"] ?? `pperk_${i}`,
      partner: r["Perk Owner"] ?? "",
      title: r["Title"] ?? "",
      category: (r["Perk Type"]?.toLowerCase() as PlayerPerk["category"]) ?? "equipment",
      description: r["Description"] ?? "",
      offer: r["Perk Offer"] ?? "",
      redeem_code: r["Perk Code"] || null,
      redeem_url: r["Perk URL"] ?? "",
  ...
  async getServices(): Promise<Service[]> {
    ...
    return rows.map((r, i) => {
      ...
      return {
        ...
        // "One-off"/"One Off"/"Once" (case-insensitive) → a flat cost;
        // anything else, including blank, defaults to "hourly" — Kennedy's
        // decision (14 Sep) to keep every service's existing behaviour
        // unless a row explicitly opts into one-off pricing.
        pricing_type: /^one[\s-]?off$|^once$/i.test((r["Pricing Type"] ?? "").trim())
          ? "one_off"
          : "hourly",
        active: true, // no active column
        created_at: new Date().toISOString().slice(0, 10),
