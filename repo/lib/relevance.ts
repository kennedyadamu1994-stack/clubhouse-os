...
/**
 * Exported (15 Sep, POS Recommendations) so lib/players/recommendations.ts
 * can classify a player's own gender/age-band the same way this module
 * classifies a club's — additive only, this function's own logic is
 * completely unchanged, so every existing CHOS call site (all internal
 * to this file) behaves identically. Kept here rather than duplicated
 * so the real keyword lists (FEMALE_KEYWORDS etc.) only ever live in
 * one place.
 */
export function classifyGender(value: string): GenderBucket | null {
  const v = norm(value);
...
/** Exported (15 Sep) — see classifyGender's own doc comment above for why. */
export function audienceFromAge(age: number): AudienceBucket {
...
/** Exported (15 Sep) — see classifyGender's own doc comment above for why. */
export function audienceFromTags(tags: string[]): AudienceBucket | null {
  for (const t of tags) {
...
/** Exported (15 Sep) — see classifyGender's own doc comment above for why. */
export function genderFromTags(tags: string[]): GenderBucket | null {
  for (const t of tags) {
...
/** Exported (15 Sep) — see classifyGender's own doc comment above for why; this one is genuinely generic already, no Club dependency. */
export const includesLoose = (list: string[], value: string): boolean => {
...
export interface RelevanceInput {
  ...
  gender?: string | null;
  /** Entry's own tags — used as the fallback source for Audience/Gender when no dedicated field is given, and as the only source for entities that don't have one (Sponsorship, Brand, Influencer, ClubDirectoryEntry). */
  tags?: string[];
}
