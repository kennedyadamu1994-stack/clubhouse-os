...
export interface InboxMessage {
  ...
  club_id: string; // resolved server-side by matching "Club" (a name) against Club.name — see doc comment above
  sent_at: string; // "Date"
...
/**
 * Real "Header Order"/"Image URL"/"URL" columns on the HEADER tab, CHOS
 * Workspace (confirmed by Kennedy, 4 Sep) — the header carousel's real
 * image source. A shared list every club rotates through, not one row
 * per club. Replaces DEMO_CAROUSEL_IMAGES, a hardcoded placeholder array
 * that stood in after an earlier hotlinked-image version broke in
 * production and was never reconnected to a real sheet-backed source.
 */
export interface HeaderImage {
  order: number; // "Header Order" — "#1" etc., parsed to a plain number for sorting
  image_url: string;
  /** "URL" column — where this slide links to when clicked. Optional: a blank cell means the slide just isn't clickable. */
  url: string | null;
}
