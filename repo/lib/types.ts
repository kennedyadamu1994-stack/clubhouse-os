...
export interface Sponsorship {
  ...
  /** Real "U notes" column, SPONSORSHIPS/FUNDING, parsed via parseRecommendedForClubIds — see Player.recommended_for_club_ids' own doc comment for the full reasoning. */
  recommended_for_club_ids: string[];
}

/**
 * From the real OPPORTUNITIES tab in "THE ULTIMATE NBRH CLUB HOUSE OS
 * DATABASE" — sports/exercise-related jobs, volunteering, and
 * placements for players, shown on the POS Jobs page (Kennedy, 15 Sep).
 * The app read this tab once, then stopped on 9 Sep when the CHOS
 * Outreach → Opportunities subsection was removed; it's being re-wired
 * here for POS, a different surface, not restored to CHOS.
 *
 * Follows the same shape as Sponsorship (opportunity_id, tags,
 * credibility_score, sponsored, recommended_for_club_ids) since it's
 * the closest existing real-column analog, plus the extra fields real
 * to this sheet that Sponsorship doesn't have.
 *
 * PREMIUM/CORE is deliberately NOT read into this type — Kennedy
 * confirmed (15 Sep) every opportunity shows to every player
 * regardless, since POS players don't have club plan tiers to gate by.
 */
export interface Opportunity {
  ...
  sponsored: boolean;
  title: string;
  /** Real "Type" column — free text (e.g. "Volunteering", "Coaching Job", "Internship"), not a fixed enum on the sheet. */
  type: string;
  sports: string[];
  provider: string; // real "Organisation / Club" column
  area: string; // real "Location (Borough)" column
  /** Real "Frequency" column — free text (e.g. "Weekly", "One-off", "Ongoing"). */
  frequency: string;
  description: string;
  requirements: string; // real "Requirements / Qualifications" column
  experience_level: string;
  dbs_required: boolean;
  paid: boolean; // real "Paid / Unpaid" column
  start_date: string;
  closing_date: string; // real "Deadline" column
  hours_per_week: string; // free text on the sheet, not guaranteed numeric — kept as string rather than forced through toNumber
  contact_name: string;
  contact_email: string;
  apply_url: string; // real "Application Link / URL" column
  status: string; // real "Status" column — free text, not assumed open/closed only
  notes: string;
  image_url: string | null;
  tags: string[];
  credibility_score: number | null;
...
export interface Player {
  ...
  name: string | null;
  /**
   * Real "Email" column, NEIGHBOURS (O) — added 15 Sep for POS's
   * email-based favourites/"For You" login (Kennedy: enter email each
   * visit, no persistent token, separate from the player's own private
   * link). ALWAYS null through gatePlayer() (lib/data/index.ts) — the
   * same never-expose rule as name, stricter than gender/age, since an
   * email address is directly contactable, unlike an anonymised
   * demographic fact. The only legitimate path to a real email is
   * getPlayerByEmail() (lib/data/index.ts), which looks up by exact
   * email match and is never reachable from a club-facing route — the
   * same deliberate exception shape as getPlayerByToken already uses
   * for name (see that method's own doc comment) — because a player
   * proving they hold their own inbox is the one legitimate case.
   */
  email: string | null;
  ...
  credibility_score: number | null;
  /**
   * Real "U notes" column, NEIGHBOURS (O), parsed via
   * parseRecommendedForClubIds (7 Sep — Kennedy's manual recommendation
   * system, replacing the old automatic chip-matching entirely). A list
   * of Club IDs this specific row should be recommended to on their
   * Overview page. Never shown to a club directly — only used to decide
   * whether this row appears in Top Recommendations.
   */
  recommended_for_club_ids: string[];
}
...
export interface Person {
  ...
  tags: string[];
  /**
   * Real "bio" column, PEOPLE sheet (confirmed in docs/schema.md's
   * real-column reconciliation, wired in 7 Sep at Kennedy's explicit
   * request: "include their Bio... in the expanded people section").
   * CONTENT CREATORS has no documented equivalent column, so rows from
   * that sheet default this to null, same honest-default pattern
   * already used for verified/direct_contact_url/credibility_score
   * above on this same source.
   */
  bio: string | null;
  /** Real "U notes" column, PEOPLE/CONTENT CREATORS, parsed via parseRecommendedForClubIds — see Player.recommended_for_club_ids' own doc comment for the full reasoning. */
  recommended_for_club_ids: string[];
}
...
export interface Brand {
  ...
  credibility_score: number | null;
  /**
   * Real "Marketing Budget Signal" column, BRANDS sheet (7 Sep, Kennedy's
   * request for more context in the expanded panel). A rough indicator
   * of the brand's real marketing spend — genuinely new to this app,
   * never read before this fix. Null when the row has no value set,
   * shown conditionally, never as an empty line.
   */
  marketing_budget_signal: string | null;
  /**
   * Real "CSR/ESG Sport Mandate" column, BRANDS sheet (7 Sep, same
   * request) — whether/how this brand's corporate social responsibility
   * or ESG commitments specifically cover sport. Genuinely new to this
   * app, never read before this fix.
   */
  csr_esg_sport_mandate: string | null;
  /** Real "U notes" column, BRANDS/BUSINESSES, parsed via parseRecommendedForClubIds — see Player.recommended_for_club_ids' own doc comment for the full reasoning. */
  recommended_for_club_ids: string[];
}
...
export interface Influencer {
  ...
  credibility_score: number | null;
  /** Real "U notes" column, INFLUENCERS, parsed via parseRecommendedForClubIds — see Player.recommended_for_club_ids' own doc comment for the full reasoning. */
  recommended_for_club_ids: string[];
}
...
export interface ClubDirectoryEntry {
  ...
  credibility_score: number | null;
  /** Real "U notes" column, Club Hub sheet, parsed via parseRecommendedForClubIds — see Player.recommended_for_club_ids' own doc comment for the full reasoning. */
  recommended_for_club_ids: string[];
}
...
export interface Supplier {
  ...
  tags: string[]; // "tags"
  /** Real "U notes" column, parsed via parseRecommendedForClubIds — see Player.recommended_for_club_ids' own doc comment for the full reasoning. Previously typed as internal_notes/private-notes (a field that, per Kennedy, 7 Sep, was never actually used for that purpose) — repurposed rather than duplicated, since it already read this exact column. */
  recommended_for_club_ids: string[];
...
/**
 * Player Inbox message (15 Sep) — real P INBOX tab, CHOS Workspace
 * workbook, a genuinely separate sheet from INBOX above. Real columns
 * (Kennedy, 15 Sep): Entry ID, Date, Club, Title, Message, URL, Email —
 * routes by the real Email column directly (unlike InboxMessage's own
 * club_id, which had to fall back to fragile name-matching because no
 * stable ID column existed on INBOX at the time it was first built — P
 * INBOX already has a genuinely reliable Email column from the start,
 * so no such fallback is needed here). The real "Club" column on P
 * INBOX is read but not currently surfaced anywhere — kept on the row
 * in case it's useful context later, not because a player message is
 * expected to reference a specific club by name as a matching key.
 */
export interface PlayerInboxMessage {
  message_id: string; // "Entry ID"
  email: string; // matched directly against the real "Email" column
  sent_at: string; // "Date"
  title: string; // "Title"
  message: string; // "Message"
  link: string | null; // "URL"
  club: string | null; // "Club" — context only, not used for matching
}
...
export interface SocialVenue {
  ...
  tags: string[];
  /** Real "U notes" column, parsed via parseRecommendedForClubIds — see Player.recommended_for_club_ids' own doc comment and Supplier.recommended_for_club_ids for the full reasoning. */
  recommended_for_club_ids: string[];
  plan_gated: boolean; // "PREMIUM/CORE"
  sponsored: boolean; // "SPONSORED?"
}
...
/**
 * From SERVICES' real "Pricing Type" column (Kennedy, 14 Sep: some
 * services are a flat one-off cost, not billed per hour). "hourly" is
 * the default when the column is blank or holds anything unrecognised,
 * matching every service's real behaviour before this column existed.
 */
export type ServicePricingType = "hourly" | "one_off";
...
export interface Service {
  ...
  description: string;
  /**
   * The service's price. Despite the field name, this is NOT always an
   * hourly rate — check pricing_type before appending "/hr" anywhere;
   * kept named hourly_rate_gbp (rather than renamed) to avoid touching
   * every existing read site for what is, for most services, still
  ...
  created_at: string;
  /**
   * Premium-tier discounted rate, when discounted from hourly_rate_gbp.
   * From SERVICES' real "Discount if on Premium Plan" column (20 Aug
   * reconciliation) — Kennedy asked for this wired in as a genuine
   * feature, not left unused. null = no discount for this service; the
   * Services page shows the discounted rate only to clubs on the
   * Premium plan, standard rate to Core. Applies to one-off services
   * too — the column doesn't distinguish, and there's no reason a
   * one-off price can't also carry a Premium discount.
   */
...
/**
 * Player perks (15 Sep) — a genuinely separate real sheet (P PERKS, CHOS
 * Workspace workbook) from PERKS above, not a re-read of the same data;
 * Kennedy: "source data from the same workbook the CHOS takes from but
 * taken from the P PERKS worksheet." No plan_tiers field at all — the
 * real PERKS sheet already has no plan-tier column either (see
 * getPerks(), lib/data/sheets.ts: "no plan-tier column — every perk
 * visible to all plans" is hardcoded there already), and Kennedy
 * confirmed (15 Sep) P PERKS should work the same way: every perk
 * available to every player, nothing locked — no plan-tier concept
 * exists for players at all, unlike clubs.
 */
...
export interface NbrhUpdate {
  ...
  note: string;
  url: string | null;
  /** Real "Image_URL" column, FROM THE NBRH tab, CHOS Workspace (16 Sep, Kennedy: added so each bulletin card can show a real image). Null when the row has no image — not every update needs one. */
...
export interface Recommendation {
  kind: "sponsorship" | "player" | "person" | "brand" | "influencer" | "club" | "supplier" | "social_venue";
  id: string;
  title: string;
  subtitle: string;
  /**
   * Reason chips explaining why this was recommended. Empty for every
   * manually-tagged recommendation (7 Sep — the "U notes" system
   * replaced the old automatic chip-matching entirely, per Kennedy's
   * explicit request), since a manual pick has no algorithmic reason to
   * show — ReasonChips already renders nothing for an empty array, the
   * same honest "no reason to show" behaviour used everywhere else in
   * the app rather than inventing a fake reason.
   */
...
/**
 * One saved favourite, as returned from storage (Kennedy's request, 9
 * Sep). A genuine SNAPSHOT of the item's display details captured at
 * the moment it was favourited (title/subtitle/href), not a live
 * reference — see lib/data/postgres.ts's ensureFavouritesTable doc
 * comment for the full reasoning, including why a snapshot is the only
 * way to honour Kennedy's explicit decision that a favourite should
 * "still show plainly" even if the underlying sheet row is later
 * deleted.
 */
