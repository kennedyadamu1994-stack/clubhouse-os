...
/**
 * Server Action wrapper around getPlayerNbrhUpdates (lib/data/*.ts) —
 * "From The NBRH" for Home. Originally (15 Sep) this read CHOS's own
 * "FROM THE NBRH" tab directly ("take from the same data source", per
 * Kennedy at the time); as of 17 Sep it reads POS's own genuinely
 * separate "P FROM THE NBRH" tab instead, so Kennedy can curate what
 * players see independently of what clubs see. Still a thin
 * passthrough, not new data logic — the wrapper exists only because
 * Home is a client component and needs a callable Server Action, the
 * same reason every other player.ts export here exists.
 */
export async function getNbrhUpdatesAction() {
  const db = getAdapter();
  return db.getPlayerNbrhUpdates();
}
...
export async function submitPlayerAction(input: {
  ...
  await notifyKennedyOfAction({
    ...
    notes: input.message,
    // 18 Sep — player submissions now go to their own P NOTIFICATIONS tab
    // (Kennedy's new worksheet, same "workspace" workbook) rather than
    // sharing NOTIFICATIONS with club rows. See notifyKennedyOfAction's
    // own doc comment in lib/actions.ts.
    tab: "P NOTIFICATIONS",
  }).catch(() => {});
