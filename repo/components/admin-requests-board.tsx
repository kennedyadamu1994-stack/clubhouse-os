...
/** Same PAGE_SIZE/see-more-see-less mechanic as OutreachList and PaginatedList (Kennedy, 1 Sep: "every list in the OS") — this board already owns its own filter/sort state, so pagination is inlined here rather than wrapped, the same way OutreachList does its own. */
const PAGE_SIZE = 5;
...
export function AdminRequestsBoard({ rows }: { rows: ResolvedRequestRow[] }) {
  ...
  // A fresh filter/sort always starts back at the first page — same
  // reasoning as OutreachList's identical effect.
  useEffect(() => {
    setVisibleCount(PAGE_SIZE);
  }, [clubFilter, statusFilter, sortBy]);

  const visible = filteredSorted.slice(0, visibleCount);
  const canSeeMore = visibleCount < filteredSorted.length;
      ...
      {filteredSorted.length === 0 ? (
        ...
        <>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {visible.map(({ row, clubName, actionLabel, detail, formattedDate }) => (
            ...
            <div className="see-more-row">
              {canSeeMore && (
                <button
                  ...
                  onClick={() => setVisibleCount((v) => Math.min(v + PAGE_SIZE, filteredSorted.length))}
                >
                  See more
                </button>
              )}
