 ...
 *
 * Real images (17 Sep, Kennedy: added an "Image_URL" column to the
 * real FROM THE NBRH sheet) — shown full-width at the top of each
 * card when present (u.imageUrl), matching the reference layout
 * Kennedy attached: image spans the card's own top edge-to-edge, no
 * padding/border interrupting it; the pink left-accent border and the
 * card's own padding apply only to the text portion underneath
 * (.nbrh-bulletin-body), not the image. Not every update needs an
 * image — the column can be blank, and the card just starts with the
 * text portion directly in that case, same as before this change.
 */
