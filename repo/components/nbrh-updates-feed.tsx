 ...
 * only (Kennedy: no auto-advance) — with square images (Kennedy's
 * explicit ask) rather than the old 160px-tall crop.
 *
 * Card layout is a fixed-size square image beside the note text
 * (side-by-side), not a full-width square stacked above the text — an
 * earlier version of this made the image 100% of the card's own
 * width, and since the card itself has no max-width (it fills a wide
 * content column), "square" ended up several hundred pixels tall,
 * pushing the note almost entirely off-screen. A real screenshot
 * caught this; see .nbrh-bulletin-card's own comment in globals.css.
 *
 ...
 *
 * Arrows are rendered ONCE, as siblings of the track (not once per
 * slide inside each card) — now that the card is a side-by-side row
 * rather than a full-width square, there's no single per-slide box to
 * overlay them on; flanking the whole carousel row is simpler and
 * correct regardless of how tall any given slide's text makes it.
 *
