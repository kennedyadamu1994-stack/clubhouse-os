  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      grouped.forEach((sessions) => {
        ...
        const label =
          sessions.length > 1
            ? `<span class="nbrh-map-marker-emoji">${icon}</span><span class="nbrh-map-marker-count">${sessions.length}</span>`
            : `<span class="nbrh-map-marker-emoji">${icon}</span>`;
      ...
      const useClustering = markers.length > MARKER_CLUSTER_THRESHOLD;
      if (useClustering) {
        ...
        const Lany = L as unknown as { markerClusterGroup: (opts?: object) => FeatureGroup };
        layer.clearLayers();
        const clusterGroup = Lany.markerClusterGroup();
        markers.forEach((m) => clusterGroup.addLayer(m));
        layer.addLayer(clusterGroup as unknown as Marker);
      }
