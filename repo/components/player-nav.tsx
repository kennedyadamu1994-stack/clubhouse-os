...
import { Home, Search, Calendar, Briefcase, Sparkles, Heart, Inbox, Map, Gift, BookOpen, ShoppingBag, Mail, type LucideIcon } from "lucide-react";
...
const PLAYER_SECTION_TITLES: Record<string, { label: string; Icon: LucideIcon }> = {
  ...
  // 19 Sep — these 6 were missing entirely (Kennedy: "for the favourites,
  // Inbox, Map, Perks, Guide, Club Store and Contact us pages, make sure
  // the header reflects the title of the page and has an Icon"). Each
  // page fell through to PlayerPageTitle's own final `return <>Home</>`
  // fallback below, so every one of these pages was actually showing a
  // bare "Home" label with no icon at all, not its own title. Icons
  // match the exact same ones PlayerHeader's own dropdown menu
  // (components/player-header.tsx) already uses for these same 6
  // destinations, so the icon language is consistent between the menu
  // and the page header rather than introducing a second, different set.
  inbox: { label: "Inbox", Icon: Inbox },
  map: { label: "Map", Icon: Map },
  perks: { label: "Perks", Icon: Gift },
  guide: { label: "Guide", Icon: BookOpen },
  store: { label: "Club Store", Icon: ShoppingBag },
  contact: { label: "Contact Us", Icon: Mail },
};
 ...
 *
 * ThemeToggle moved out entirely, 19 Sep (Kennedy: "the light/dark mode
 * toggle, put it in the footer") — it used to render twice here (once
 * under the carousel on Home via .deck-toggle-row, once inline in
 * deck-head-controls on every other page); now lives once in
 * components/app-footer.tsx instead, so .deck-toggle-row and the now-
 * empty deck-head-controls wrapper are both removed along with it.
 */
