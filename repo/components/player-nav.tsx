 ...
 *
 * playerToken is now OPTIONAL (15 Sep fix) — every link here originally
 * assumed a token always exists, but the public route tree
 * (/players/search|calendar|jobs, no token) has no such thing. Passing
 * playerToken builds /players/[token]/... links (the personal tree);
 * omitting it builds plain /players/... links (the public tree) —
 * Recommendations is the one section that has no meaningful public
 * version at all (there's no profile to recommend from without a
 * player), so it's simply left out of the nav when there's no token.
 */
const POS_SECTIONS: { slug: string; label: string; icon: LucideIcon; personalOnly?: boolean }[] = [
  ...
  { slug: "recommendations", label: "Recommendations", icon: Sparkles, personalOnly: true },
];
...
export function PlayerNavLinks({ playerToken }: { playerToken?: string }) {
  ...
  const base = playerToken ? `/players/${playerToken}` : "/players";
  const sections = playerToken ? POS_SECTIONS : POS_SECTIONS.filter((s) => !s.personalOnly);
 ...
 * (15 Sep fix) — /players/[token]/section has it at segments[2], the
 * public /players/section has it at segments[1], since there's no
 * token segment in between. Checking both rather than assuming one
 * fixed depth is what makes this work correctly on either tree.
 */
