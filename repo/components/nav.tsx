...
import { Home, Users, LayoutGrid, Wrench, Sparkles, UserCircle } from "lucide-react";
...
export function NavLinks({ clubToken }: { clubToken: string }) {
  ...
  return (
    <>
      ...
      <Link
        className="navlink"
        href={href(clubToken, "membership")}
        aria-current={pathname.startsWith(`${base}/membership`) ? "page" : undefined}
      >
...
export function TabBar({ clubToken }: { clubToken: string }) {
  ...
  const isMembership = pathname.startsWith(`${base}/membership`);
  ...
  return (
    <nav className="tabbar" aria-label="Sections">
      ...
      <Link href={href(clubToken, "membership")} aria-current={isMembership ? "page" : undefined}>
