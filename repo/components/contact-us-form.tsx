...
export function ContactUsForm({
  ...
  club_id: string;
  onClose: () => void;
  /** Pre-fills the message field — e.g. the insufficient-tokens upgrade prompt (28 Aug) opens this with a ready-made request instead of a blank form. */
      ...
      <div className="modal" role="dialog" aria-modal="true" aria-labelledby={titleId} ref={dialogRef} tabIndex={-1}>
        ...
        {isDone ? (
          ...
          <>
            ...
            {status.state === "error" && (
              <p style={{ color: "var(--danger)", fontSize: "0.82rem", marginBottom: 12 }} role="alert">
                {status.message}
              </p>
            )}
            ...
            <button className="btn btn-pink" onClick={handleSubmit} disabled={isSubmitting}>
              {isSubmitting
                ? "Sending…"
                : TYPES.find((t) => t.value === type)?.label ?? "Send"}
            </button>
          </>
        )}
      </div>
