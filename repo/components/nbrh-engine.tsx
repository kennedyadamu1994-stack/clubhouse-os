...
interface NbrhEngineProps {
  ...
  showPeople?: boolean;
  /**
   * When given, the category tabs render as real <Link>s to
   * `${linkBase}?category=<slug>` instead of in-component setCategory()
   * buttons — this is the actual Search-into-real-pages split Kennedy
   * asked for (15 Sep: "utilises the drop down bar to change pages"
   * instead of the self-contained tool). A query param, not a URL path
   * segment (15 Sep rewrite) — Search was originally a dynamic
   * [category] route, which produced a genuine, confirmed runtime 404
   * in production (a same-named dynamic segment collided between the
   * public and personal Search trees — Next.js 15's build passes clean
   * on this shape, only the live request fails). Moving to a plain
   * static page with a query param removes the dynamic segment
   * entirely, on both trees, so there's nothing left to collide.
   * Omitted entirely for CHOS's own page, which keeps the original
   * single-page in-component tab-switching behaviour untouched.
   */
  linkBase?: string;
  /**
    ...
    <div className="sr-root">
      <div className="sr-category-tabs">
        {visibleCategories.map((cat) =>
          linkBase ? (
            // POS: a real route per category (Kennedy, 15 Sep — "utilises
            // the drop down bar to change pages" instead of the
            // self-contained tool). resetFilters() isn't called here the
            // way the button below calls it — navigating to a new route
            // unmounts and remounts this component fresh (each category
            // page passes its own initialCategory), so filters/sort/
            // search state naturally starts clean on the new page without
            // needing to be reset by hand.
            <Link
              key={cat}
              href={`${linkBase}?category=${CATEGORY_SLUG[cat]}`}
      ...
      <div className={`sr-cards ${viewMode === "list" ? "sr-cards-list" : ""}`}>
        {sorted.length === 0 ? (
          ...
          sorted.map((c) =>
            cardRenderer ? (
              <div key={c.id}>{cardRenderer(c)}</div>
            ) : (
              <EngineCard key={c.id} item={c} category={category} onDetails={() => setDetailItem(c)} />
            ),
          )
        )}
      </div>
