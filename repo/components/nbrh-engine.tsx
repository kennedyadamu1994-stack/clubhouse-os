...
function PaginatedListForSearch({ items, cardRenderer }: { items: Item[]; cardRenderer: (item: Item) => React.ReactNode }) {
  ...
  // "entry-list sr-entry-list" (19 Sep) — entry-list alone is shared with
  // every OTHER list that reuses .entry-row (Favourites, Trending,
  // Outreach — see EntryCard's own usage elsewhere), which sit inside
  // .outreach-card and were never asked to go full-bleed on mobile.
  // sr-entry-list is the one distinguishing hook the mobile full-bleed
  // CSS rule targets, so ONLY this search-results usage of .entry-row
  // bleeds edge-to-edge — matching the same precision Kennedy asked for
  // with .sr-cards, not a blanket change to every entry-row in the app.
  return <PaginatedList items={items.map((c) => <div key={c.id}>{cardRenderer(c)}</div>)} className="entry-list sr-entry-list" />;
}
