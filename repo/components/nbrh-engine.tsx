...
function EngineCard({ item: c, category, onDetails }: { item: Item; category: Category; onDetails: () => void }) {
  ...
  return (
    <div className={`sr-card ${cardClass}`}>
      <div className="sr-img-container">
        {eventDateBadge && <div className="sr-event-date-badge">{eventDateBadge}</div>}
        {typeBadge && <div className="sr-type-badge">{typeBadge}</div>}
        {category !== "Activities" && (
