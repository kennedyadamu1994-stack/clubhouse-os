 ...
 * lib/players/recommendations.ts reuses this rather than building a
 * second client-side fetch against the same opensheet.elk.sh endpoint,
 * which would risk drifting out of sync with this one (different
 * caching behaviour, different error handling) over time.
 */
 ...
 * Recommendations, lib/players/recommendations.ts, reuses the real
...
export function NbrhEngine({ initialCategory, initialSearch, showPeople, linkBase, cardRenderer }: NbrhEngineProps = {}) {
  const router = useRouter();
  const visibleCategories = useMemo(
    () => ALL_CATEGORIES.filter((c) => TAB_VISIBILITY[c] === "Open" || (c === "People" && showPeople)),
    [showPeople],
  );
  ...
  // Same initialSearch → search sync issue as category above, same
  // fix — useState's own initial value is only read on first mount.
  useEffect(() => {
    if (linkBase && initialSearch !== undefined && initialSearch !== search) {
      setSearch(initialSearch);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- only initialSearch/linkBase should trigger this, same reasoning as the category sync effect above
  }, [initialSearch, linkBase]);
  const [sortKey, setSortKey] = useState<SortKey>("default");

  // Re-syncs `category` whenever initialCategory changes (15 Sep real
  // bug fix — Kennedy: "I can't click and see Clubs, Leagues, Venues,
  // Events or People"). useState's own initial-value argument is only
  // ever read on this component's FIRST mount; it's silently ignored
  // on every later render. Since NbrhEngine is a client component
  // nested inside the Search server page, and Next.js's App Router
  // does NOT tear down and remount an already-mounted client component
  // just because its server-component parent re-ran with a new prop
  // from a URL change, clicking a different category tab changed the
  // URL and the server page's own computed initialCategory correctly,
  // but this component's own internal category state never picked up
  // the new value — it kept showing whichever category was active on
  // first load, regardless of which tab was clicked. (An earlier doc
  // comment on the POS/(browse)/search page claimed navigating to a
  // new route "unmounts and remounts this component fresh" — that
  // assumption was never actually verified and was wrong; this effect
  // is the real fix, not a route-level remount.)
  useEffect(() => {
    if (linkBase && initialCategory && initialCategory !== category) {
      setCategory(initialCategory);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- only initialCategory/linkBase should trigger this; including `category` itself would re-run every time this effect sets it
  }, [initialCategory, linkBase]);
