 ...
 * grid." — Opportunities was removed from this view entirely, 9 Sep;
 * Calendar now shows real Events only). One component, two presentations
 * of the same data, gated by CSS (.cal-grid / .cal-agenda) rather than
 * two separate components — same pattern as EntryCard's row layout
 * switching at the mobile breakpoint. The month grid is desktop-only;
 * mobile always gets the agenda list, per the spec's explicit warning
 * against squeezing a grid onto a phone.
 *
 ...
 * Events move from local JSON to a live Google Sheet — the seam is
 * lib/data's adapter (see lib/data/sheets.ts's own note), which
  ...
  return (
    ...
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal" role="dialog" aria-modal="true" aria-labelledby={titleId} ref={dialogRef} tabIndex={-1}>
        ...
        <span className="cal-detail-source">Event</span>
