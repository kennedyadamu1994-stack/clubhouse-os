...
export function buildDetailRows(item: Item): DetailRow[] {
  ...
  switch (item.category as Category) {
    case "Activities":
      push("Type", item.type);
      push("Difficulty", item.difficulty);
      push("Audience", item.audience);
      push("Day", item.dayOfWeek);
      if (item.startTime) push("Time", item.endTime ? `${item.startTime} – ${item.endTime}` : item.startTime);
      push("Price", item.price > 0 ? `£${item.price.toFixed(2)}` : "Free");
      break;
    case "Clubs":
    case "Leagues":
      push("Type", item.type);
