...
export function buildDetailRows(item: Item): DetailRow[] {
  ...
  switch (item.category as Category) {
    case "Activities":
      // Real field list (17 Sep, trimmed to Kennedy's exact final
      // list) — Date, Duration, Equipment Provided, Cancellation
      // Hours, Indoor/Outdoor, and Badge were all shown before this
      // and are now deliberately removed, not omitted by accident;
      // every field still here reads the same real, confirmed column
      // via mapActivity (components/nbrh-engine.tsx) as before. Club
      // Hub renders as a real link rather than plain text, matching
      // how EntryCard's own optional href field works for detail rows
