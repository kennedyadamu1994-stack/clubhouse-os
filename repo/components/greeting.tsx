 ...
 * known TITLES key). Overview gets the no-eyebrow treatment; every
 * other page gets back its original eyebrow "Hello, {clubName}" +
 * TokenWidget, unchanged from before 18 Sep. TokenWidget itself already
 * has its own separate Overview-only check (see that component's own
 * doc comment) so it's rendered here unconditionally and trusted to
 * hide itself.
 *
 ...
 * under the carousel on Overview via .deck-toggle-row, once inline in
 * deck-head-controls on every other page); now lives once in
 * components/app-footer.tsx instead, so .deck-toggle-row (which existed
 * only to hold it on Overview) is removed along with it.
 */
