 ...
 * Social Venues, Resources, FAQ, Perks, Trending Topics, Services, and
 * Tools. Opportunities was removed from this list entirely (Kennedy's
 * request, 9 Sep — the whole Workspace → Opportunities subsection was
 * removed).
 *
 ...
 * actions"). Went through two corrections the same day before landing
 * here:
 *   1. First built as Info + a plain "View" navigation link.
 *   2. Corrected to a full-text pink CTA button after Kennedy clarified
 *      he wanted a real working action, not just a link to go find one.
 *   3. Corrected AGAIN after Kennedy pointed out the FAQ result (which
 *      never had an action, only ever the plain View link) was already
 *      the right compact shape, and asked for that same shape
 *      everywhere: "don't implement the full CTA text... have the
 *      action button be a white arrow in a pink box, similar in shape to
 *      the arrow in the box for the FAQ result."
 *
 * Final shape:
 ...
 *   - Action: a compact pink square with a white arrow, same size and
 *     shape as Info's own icon button. For entries with a real action
 *     (r.actions present), clicking it opens the real ActionPopup for
 *     that exact entry — genuinely working, not a link to go find one —
 *     using the same action_key/token_cost/label logic each entity's own
 *     Outreach/Workspace page already runs. For entries with no action
 *     (FAQ, Trending, Tool), the same-shaped pink arrow just navigates to
 *     the entry's page instead, since there's nothing to act on.
 */
                    ...
                    <span className="askbar-result-actions">
                      ...
                      {r.actions && r.actions.length > 0 ? (
                        <button
                          ...
                          aria-label={r.actions.find((a) => a.colour === "pink")?.label ?? r.actions[0].label}
                          title={r.actions.find((a) => a.colour === "pink")?.label ?? r.actions[0].label}
