...
import {
  loginByEmail,
  getPlayerFavouritedKeysAction,
  getPlayerFavouritesAction,
  getUnreadPlayerInboxCountAction,
...
interface PlayerSessionValue {
  ...
  login: (email: string, forceFresh?: boolean) => Promise<boolean>;
  signOut: () => void;
...
export function PlayerSessionProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshInboxCount = useCallback(async () => {
    ...
    const count = await getUnreadPlayerInboxCountAction(email);
  ...
  const login = useCallback(async (rawEmail: string, forceFresh = false): Promise<boolean> => {
    setLoading(true);
    setError(null);
    const result = await loginByEmail(rawEmail, forceFresh);
  ...
  // Fetches the real unread Inbox count once whenever the logged-in
  // email changes — same pattern as refreshFavourites above.
  useEffect(() => {
    refreshInboxCount();
    // eslint-disable-next-line react-hooks/exhaustive-deps -- refreshInboxCount itself already depends on [email], same reasoning as the favourites effect above
  }, [email]);
