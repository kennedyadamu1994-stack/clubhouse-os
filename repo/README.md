...
## Latest session: collapsible pattern removed from Outreach entries
...
avatar, name, context line, tags, compact match-score square, "i" button,
"View options" — no toggle, no collapsed state. The "i" button still
reveals a small detail strip inline (for fields that genuinely don't fit
in the row), but that's the only expand/collapse left, and it's per-field,
not a whole-card state.

Overview's `CollapsibleCard` (Profile completeness, KPIs, etc.) was left
untouched — Kennedy's feedback was specifically about the Outreach
subsections' data entries, not Overview's summary cards.
