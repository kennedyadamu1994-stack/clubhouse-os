...
- ✅ **Splash pages** — `/` links straight to Hackney Marshes FC's dashboard
  now that Riverside Netball Club (the second pilot club) has been removed
  and the pilot is down to one club. The old multi-club picker still lives
  at `/directory`, unlinked from `/`; the personalised per-club splash at
  `/dashboard/[clubToken]/welcome` is also unlinked from the default flow
  but kept working for when a second club is onboarded.
- ✅ **Workspace** — Insights (club-specific comparison + a platform-wide
  session-data block below it), Trending Topics (tailored news, each with a
  club-specific "why it matters" line), Opportunities (incl. club-submitted
  call-outs), Perks (partner benefits, plan-tier gated — a Premium-only perk
  shows locked to a Core club rather than disappearing), Resources, FAQ all
  live. Copy Generator is a designed "coming soon" tile — DECISIONS.md D5 is
  still **OPEN** and blocks it.
...
- ✅ **Services** — card grid, all 8 confirmed categories seeded, "Get in
  touch" reuses ActionPopup with a single free pink option.
- ✅ **Membership** — new top-level section (Kennedy, 17 Aug): the club's
  own details plus their current plan, token allocation, what the plan
  includes, and a comparison against the other tier. Plan changes route
  through Contact Us for now — no self-serve upgrade/downgrade write flow
  yet (spec § Overview mentions this as a future form).
...
- ⚠️ Riverside Netball Club's row was also the only seeded example of a
  brand-new club with no priorities/goals/kpis yet (CLAUDE.md rule 4 —
  every screen, including a brand-new club, needs a designed empty state).
  Removing it means that empty-state path has no seed data to test against
  right now — worth re-seeding a differently-named row for that purpose
  before onboarding a real second club.
- ⚠️ Perks placement: filed under Workspace alongside Resources/Opportunities
  (both "useful to the club" content) since the spec doesn't define this
  section — flag if a different home is wanted.
...
`/` is now the link to hand to Hackney Marshes FC — it goes straight to
their dashboard. `/dashboard/[clubToken]/welcome` still works as a
personalised splash screen if you want one for a future second club, it's
just not where `/` sends people during the single-club pilot.

Demo dashboard (unguessable token — rotate before real use):
- **Hackney Marshes FC:** `/dashboard/hkfc-x7Kq2mZ9pR4wT8vN3sJ6`
...
### Header banner fix + three new sections: Perks, Trending Topics, Membership
The header carousel's images were broken in production — the carousel
mechanism (built two sessions ago) used hotlinked Unsplash URLs, and a
hotlinked photo can stop resolving at any time (deleted, ID changed,
CDN rate-limited) with nothing on our end to fix it. Rather than swap in
different external URLs I couldn't verify would stay up either, replaced
them with five bundled SVG banners in `public/banners/` — self-contained,
on-brand (dark background, hot-pink accent, five distinct motifs: pitch/
centre-circle, stadium arcs, diagonal energy, network graph, horizon
line), and structurally incapable of going dead the way an external
hotlink can. `headerImageFor()` (the real per-club photo logic) is
untouched; only the carousel's demo image set changed.

Three new sections, each following the existing data-adapter → seed-JSON →
page → nav pattern used throughout:

- **Perks** (Workspace): 6 demo partner benefits across equipment, food &
  drink, software, training, travel, wellbeing. Gated by `plan_tiers` —
  a perk not available on the club's current tier still renders, dimmed,
  with a "Premium" badge and an upgrade prompt instead of the redeem
  button, rather than disappearing (no dead ends). Redeeming is a plain
  external link, not a token action — a perk is a standing benefit, not
  a request for Kennedy's time.
- **Trending Topics** (Workspace): 6 demo news stories, each with a
  `why_it_matters` line written to reference Hackney's actual seeded
  goals (youth section, kit sponsor, facilities) — shown inline in a
  highlighted callout rather than behind a toggle, since that's the
  entire point of the section per Kennedy's brief ("tailored for the
  club's needs"). Sorted newest-first.
- **Membership**: new top-level section (own nav item, own icon, sidebar
  + mobile tab bar) — the club's own details (name, sport, area, contact,
  members, teams, join date) and their current plan (price, token
  allocation, what's included, comparison against the other tier). Plan
  pricing lives as a small constant in the page itself (app config, not
  per-club data — every club sees the same two tiers). No self-serve
  upgrade/downgrade write flow yet — both "edit details" and "change
  plan" route through the existing Contact Us modal, same as the spec's
  Overview section describes plan management as a future form, not
  something to build blind today.

Placement judgement call, flagged rather than silently decided: Perks
went under Workspace (alongside Resources/Opportunities, the other
"useful to the club" content) since the brief didn't specify a section
and Workspace was the closest existing fit. Easy to move if a different
home is wanted.
