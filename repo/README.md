...
- ✅ **Tools** — Contact Us (its own modal with a type selector), Find
  (link-out to thenbrh.co.uk — see changelog for why), Calendar (month
  grid desktop / agenda list mobile, from Events + dated Opportunities).
...
- Tools: Contact Us (`ContactUsForm` — its own modal, type selector, reuses
  the `contact_us` ledger action), Find (link-out to thenbrh.co.uk — see
  changelog), Calendar (`CalendarView` — month grid desktop / agenda list
  mobile, from `Events` + dated `Opportunities`)
...
3. Implement `lib/data/sheets.ts` against the real spreadsheet — now also
   ...
   see that file's own notes. (The platform-wide session data on Insights is
...
7. A real universal search bar and a header "help" icon linking to FAQ are
   ...
   this predates today's Tools work, not introduced by it, but worth
   scoping deliberately rather than bolting on ad hoc inside whichever
   section needs it next
8. If the NBRH Engine gets a standalone, embeddable URL (or a documented
   prefill parameter) later, `tools/find/page.tsx` is a one-file upgrade
   from link-out to embed

## Changelog

### This session: Tools built (Contact Us, Find, Calendar)
