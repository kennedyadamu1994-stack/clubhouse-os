...
interface ActionPopupProps {
  ...
  /**
   * No longer used for anything visible (29 Aug — Kennedy: "remove the
   * 'What's a token?' box... from all pop ups") — this used to gate a
   * one-off explainer shown only on a club's first pink action. Left as
   * a required prop rather than removed from every call site across the
   * app, since every page that renders ActionPopup already computes and
   * passes this value; removing it here would be a much bigger, riskier
   * change than what was actually asked for. Safe to actually delete
   * (prop + every call site) in a future pass if it's confirmed nothing
   * else will ever need it.
   */
...
export function ActionPopup({
  ...
  useEffect(() => {
    let cancelled = false;
    getClubTokenStatus(clubToken, club_id).then(({ balance, plans, currentTier }) => {
  ...
  async function handlePinkAction(opt: ActionOption) {
    ...
    try {
      ...
      // Keep the client's own idea of the balance in sync with what the
      // server just computed (Kennedy's report, 6 Sep: a popup showed
      // "costs 2, but you have 20 left" and was rejected anyway — the
      // real cause was tokenStatus never being updated after a
      // successful spend earlier in the same session, so a later
      // affordability check in this same component kept comparing
      // against the stale pre-spend number). Without this, the NEXT
      // paid action attempted here would still race server-side and hit
      // the same insufficient_tokens rejection, with a message quoting
      // an already-outdated balance.
      setTokenStatus((prev) => (prev ? { ...prev, balance: result.new_balance } : prev));
